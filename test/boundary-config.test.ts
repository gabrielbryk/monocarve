...
describe("compositionBoundaries schema refusals", () => {
  ...
  test('strategy "existing-package" missing replacement is rejected at config load', () => {
    expect(() => config({ compositionBoundaries: [{ id: "env-shim", retained: `${APP}/config/env.ts`, strategy: "existing-package" }] })).toThrow(
      /strategy "existing-package" requires a "replacement"/,
    );
  });
  ...
  test("selective existing-package boundaries cannot retire mixed donors", () => {
    ...
    ).toThrow(/selective boundaries require existing-package strategy with retire false/);
  });

  test("duplicate ids within portPromotions are rejected at config load", () => {
    expect(() =>
      config({ portPromotions: [portPromotion({ id: "db-port" }), portPromotion({ id: "db-port", appConcreteType: `${APP}/db/other.ts#Client` })] }),
...
describe("portPromotions schema refusals", () => {
  test("accepts an atomic concrete declaration group and rejects mixed singleton/group forms", () => {
    expect(
      ...
      }).portPromotions[0]?.appConcreteTypes,
    ).toHaveLength(2);

    expect(() => config({ portPromotions: [portPromotion({ appConcreteTypes: [`${APP}/db/client.ts#Other`], libraryPorts: ["Other"] })] })).toThrow(
      /exactly one of appConcreteType\/libraryPort or appConcreteTypes\/libraryPorts/,
    );
  });

  test("rejects incomplete and differently-sized declaration groups", () => {
    expect(() =>
      config({ portPromotions: [portPromotion({ appConcreteType: undefined, libraryPort: undefined, appConcreteTypes: [`${APP}/db/client.ts#Client`] })] }),
      ...
      config({
        portPromotions: [
          portPromotion({
            appConcreteType: undefined,
            libraryPort: undefined,
            appConcreteTypes: [`${APP}/db/client.ts#Client`],
            libraryPorts: ["Client", "Other"],
          }),
        ],
      }),
    ).toThrow(/same length/);
  ...
  test('appConcreteType not in "path/file.ts#TypeName" form is rejected at config load', () => {
    expect(() => config({ portPromotions: [portPromotion({ appConcreteType: `${APP}/db/client.ts` })] })).toThrow(/path\/to\/file\.ts#TypeName/);
  });

  test("appConcreteType naming a non-.ts/.tsx file is rejected at config load", () => {
    expect(() => config({ portPromotions: [portPromotion({ appConcreteType: `${APP}/db/client.js#Client` })] })).toThrow(/path\/to\/file\.ts#TypeName/);
  });
});
...
describe("resolveBoundaries", () => {
      ...
      compositionBoundaries: [
        {
          ...
          retained: `${APP}/config/env.ts`,
          strategy: "existing-package",
          replacement: { specifier: "@acme/env", symbols: ["envB", "envA"] },
          retire: true,
        },
      ],
    });
  ...
  test("a portPromotion whose libraryPort does not name the same declaration as appConcreteType is rejected", () => {
    const cfg = config({ portPromotions: [portPromotion({ libraryPort: "DifferentName" })] });
  ...
  test("rejects declaration groups spanning files or renaming a member", () => {
    const spanning = config({
      portPromotions: [
        portPromotion({
          appConcreteType: undefined,
          libraryPort: undefined,
          appConcreteTypes: [`${APP}/db/client.ts#Client`, `${APP}/db/other.ts#Other`],
          libraryPorts: ["Client", "Other"],
        }),
      ],
    });
    ...
    const renamed = config({
      portPromotions: [
        portPromotion({
          appConcreteType: undefined,
          libraryPort: undefined,
          appConcreteTypes: [`${APP}/db/client.ts#Client`, `${APP}/db/client.ts#Other`],
          libraryPorts: ["Client", "Renamed"],
        }),
      ],
    });
