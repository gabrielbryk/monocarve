...
test("preparer bootstrap commits introducing config through hooks that require generated output", async () => {
  ...
  const committed = await runJsonIn<{ commit: string }>(
    root,
    "preparer-bootstrap-commit",
    "--plan",
    planned.output,
    "--subject",
    "chore: configure guide sync",
  );
  expect(committed.commit).toHaveLength(40);
  expect(readFileSync(join(root, "generated-guide.md"), "utf8")).toBe("stale output\n");
  expect(existsSync(join(root, "generated-new.md"))).toBe(false);
...
test("preparer bootstrap fails closed and rolls back temporary outputs when a hook rejects", async () => {
  ...
  const config = JSON.parse(readFileSync(configPath, "utf8")) as Record<string, unknown>;
  config.preparers = [
    { id: "sync", phase: "pre-extraction", command: "printf generated > generated.txt", outputs: ["generated.txt"], commit: { subject: "docs: sync" } },
  ];
  writeFileSync(configPath, `${JSON.stringify(config, null, 2)}\n`);
  ...
  const planned = await runJsonIn<{ output: string }>(
    ...
    "monocarve.config.json",
    "--out",
    ".monocarve/plans/bootstrap.json",
    "--write",
  );
  ...
  config.preparers = [
    {
      id: "ratchet",
      phase: "pre-extraction",
      command: "mkdir -p libs/chart && printf ok > {packageRoot}/ratchet.txt",
      outputs: ["{packageRoot}/ratchet.txt"],
      commit: { subject: "chore: update ratchet" },
    },
  ];
  writeFileSync(configPath, `${JSON.stringify(config, null, 2)}\n`);
  fixtureGit(root, "add", "--", "monocarve.config.json");
...
test("preparer CLI plans and simulates a command-free declarative create", async () => {
  ...
  const planned = await runJsonIn<{ output: string; mutations: { path: string; preconditionHash: string; resultMode: number }[] }>(
    root,
    "preparer-plan",
    "--extraction",
    extractionPath,
...
test("preparer-plan keeps noisy successful disposable installs out of JSON stdout", async () => {
  ...
  const child = Bun.spawn(
    ...
    { cwd: ROOT, env: { ...process.env, PATH: `${bin}${delimiter}${process.env.PATH ?? ""}` }, stdout: "pipe", stderr: "pipe" },
  );
