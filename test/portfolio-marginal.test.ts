...
describe("marginalBlockers", () => {
  test("ranks candidates a single concrete edge really frees, not an often-seen multi-blocker", () => {
    const escapes: RejectionReason = { code: "closure-escapes-app-code", detail: "imports leave the closure", edges: ["apps/a.ts -> ../hub.ts"] };
    ...
    expect(result).toEqual([
      ...
      { code: "dynamic-imports", detail: "dynamic", occurrences: 1, freedCandidates: 0, freedLineCount: 0, candidateIds: [] },
      { code: "too-large", detail: "large", occurrences: 1, freedCandidates: 0, freedLineCount: 0, candidateIds: [] },
    ]);
  });

  test("does not credit either edge when one rejection itself requires two edges", () => {
    const result = marginalBlockers([
      candidate("c-two-edges", 200, [{ code: "closure-escapes-app-code", detail: "two escapes", edges: ["apps/a.ts -> ../one.ts", "apps/a.ts -> ../two.ts"] }]),
    ]);
