...
test("allow-empty cannot conceal a missing source root", async () => {
  ...
  const result = await qualifyWorkspace({ config: qualificationConfig(), rootDir: root, application: "web", reports: { web: { modules: [] } }, allowEmpty: true });
...
test("an unrecognized qualification error defaults to fatal", () => {
  const result = qualifyAssessment({ diagnostics: [{ code: "WORKSPACE_DISCOVERY_FAILED", severity: "warning", message: "unreadable", impact: "unavailable" }] });
...
test("allow-empty cannot override replay mismatch or input drift", () => {
  for (const code of ["ASSESSMENT_REPLAY_INPUT_MISMATCH", "ASSESSMENT_INPUT_DRIFT"] as const) {
    const result = qualifyAssessment({ allowedEmpty: true, diagnostics: [{
      code, severity: "warning", message: "authority changed", impact: "captured evidence is stale",
    }] });
...
test("a missing or inconsistent scanner report is fatal instead of an allowed empty", async () => {
  ...
  const inconsistent = await qualifyWorkspace({
    config: qualificationConfig(), rootDir: root, application: "web", allowEmpty: true,
    reports: { web: { modules: [
...
test("fatal assessment JSON has the complete machine-readable outcome envelope", async () => {
  ...
  const result = await runIn(join(import.meta.dir, "../fixtures/basic-monorepo"), "assess", "--app", "web", "--evidence-dir", "fatal-output", "--replay", "missing-replay", "--json");
...
test("missing source roots produce fatal JSON and no evidence directory", async () => {
  ...
  expect(JSON.parse(result.stdout)).toMatchObject({ status: "fatal", exitCode: 1, published: false, diagnostics: [expect.objectContaining({ code: "SOURCE_ROOT_MISSING" })] });
