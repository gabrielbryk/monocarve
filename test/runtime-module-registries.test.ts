...
const declaration = { file: "apps/api/config/route-registry.json", pointer: "/domains/*/module", resolveFrom: "apps/api/src" };

test("rewrites only declared runtime module fields relative to their declared resolver root", () => {
  const text = JSON.stringify({ note: "./sales/routes.ts", domains: [{ id: "sales", module: "./sales/routes.ts", export: "salesRoutes" }] }, null, 2) + "\n";
  ...
  const rewrites = scanRuntimeModuleRegistry(unique, declaration, [{ source: "apps/api/src/sales/routes.ts", target: "libs/sales-runtime/src/routes.ts" }]);
...
test("refuses a selected registry value without a byte-unique location", () => {
  ...
  expect(() => scanRuntimeModuleRegistry(text, declaration, [{ source: "apps/api/src/sales/routes.ts", target: "libs/sales-runtime/src/routes.ts" }])).toThrow(
    "runtime module registry value is not byte-unique",
  );
});
...
test("preserves an exact prefix stripped by the modeled runtime before resolution", () => {
  ...
  const rewrites = scanRuntimeModuleRegistry(text, { ...declaration, stripPrefix: "./" }, [
    { source: "apps/api/src/sales/routes.ts", target: "libs/sales-runtime/src/routes.ts" },
  ]);
  expect(rewrites[0]?.to).toBe("./../../../libs/sales-runtime/src/routes.ts");
  expect(rewrites[0]?.strippedPrefix).toBe("./");
  expect(posix.normalize(posix.join(declaration.resolveFrom, rewrites[0]!.to.slice(2)))).toBe("libs/sales-runtime/src/routes.ts");
});
