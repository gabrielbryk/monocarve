...
describe("preparation manifest compiler", () => {
  ...
  test("refuses a seam whose source evidence is stale", () => {
    ...
    expect(() => compilePreparationManifest({ ...input, seam: { ...input.seam, sourceHash: hashText("concurrent edit") } })).toThrow("does not match the resolved baseline donor");
  });
  ...
  test("refuses when dependency closure expands beyond the reviewed seam", () => {
    ...
    expect(() => compilePreparationManifest(buildInput(root, config, source, [reviewed.id])))
      .toThrow("dependency closure expands beyond the operator-reviewed seam");
  });
  ...
  test("compiles declared interfaces and type aliases while retaining their ambient bytes", () => {
    ...
    expect(ts.transpileModule(operation.targetContents, { compilerOptions: { module: ts.ModuleKind.ESNext } }).outputText)
      .toBe("export {};\n");
  });
  ...
  test("refuses unresolved commit-subject placeholders instead of creating a plan-id cycle", () => {
    ...
    expect(() => compilePreparationManifest({
      ...input,
      rendering: { ...input.rendering, commit: { subject: "chore: prepare {planId}" } },
    })).toThrow("must be rendered");
  });
  ...
  test("persists and replays exact provenance for relative inline import types", () => {
    ...
    expect(operation.inlineImportTypeProofs).toEqual([{
      originalSpecifier: "./decimal.ts", targetSpecifier: "../decimal.ts", resolvedSourcePath: "src/decimal.ts",
      start: source.indexOf(literal), end: source.indexOf(literal) + literal.length,
      sourceHash: hashText(literal), proofBaselineHash: hashText(source),
    }]);
...
function buildInput(root: string, config: ReturnType<typeof fixtureConfig>, sourceText: string, groupIds?: readonly string[]) {
  ...
  const movedGroups = groupIds === undefined
    ? graph.groups.filter((group) => group.space === "type")
    : graph.groups.filter((group) => groupIds.includes(group.id));
  const seam = {
  ...
  return {
    ...
    rendering: {
      commit: { subject: "chore: prepare declaration seam" },
