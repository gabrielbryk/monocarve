...
test("attributes an undeclared gate output without touching the checkout", async () => {
  ...
  expect(report.effects).toEqual([
    expect.objectContaining({ tier: "workspace", exitCode: 0, changedPaths: ["reports/index.txt"], undeclaredPaths: ["reports/index.txt"] }),
  ]);
  expect(report.effects[0]?.suggestedArtifacts[0]).toEqual({
    path: "reports/index.txt",
    regenerate: manifest.gates.workspace[0]!,
    triggers: manifest.changedFiles,
  });
...
test("isolates every gate even when an earlier gate fails", async () => {
  ...
  const manifest = {
    ...baseManifest(root),
    gates: { package: ["printf first > first.txt; exit 7", "test ! -e first.txt && printf second > second.txt"], project: [], workspace: [] },
  };
  const report = await inspectGateEffects({ config, rootDir: root, manifest });
