...
test("target campaigns re-resolve a stable source path and remain read-only by default", async () => {
  ...
  writeFileSync(
    join(workspace, "campaign.targets.json"),
    JSON.stringify({ application: "web", targets: [{ path: "apps/web/src/widgets/chart.ts", packageName: "@acme/chart-ui" }] }),
  );
  ...
  const result = JSON.parse(first.stdout) as {
    outcome: string;
    written: boolean;
    targets: { path: string; outcome: string; candidateId?: string }[];
    manifest: { target: { packageName: string } };
  };
...
test("target campaigns reject duplicate stable identities before compiling", async () => {
  const workspace = committedWorkspace();
  writeFileSync(
    join(workspace, "campaign-duplicates.targets.json"),
    JSON.stringify({
      application: "web",
      targets: [
