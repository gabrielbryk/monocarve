...
test("input inventory rejects source membership, consumer, config, package export, and symlink drift", () => {
  ...
  assertDrift(
    () => writeFileSync(join(root, "apps/web/src/added.ts"), "export {};\n"),
    () => unlinkSync(join(root, "apps/web/src/added.ts")),
  );
  assertDrift(
    () => writeFileSync(join(root, "apps/web/src/consumer.ts"), "export const value = 2;\n"),
    () => writeFileSync(join(root, "apps/web/src/consumer.ts"), "export const value = 1;\n"),
  );
  assertDrift(
    () => writeFileSync(join(root, "apps/web/tsconfig.json"), '{"include":[]}\n'),
    () =>
      writeFileSync(join(root, "apps/web/tsconfig.json"), '{"compilerOptions":{"allowImportingTsExtensions":true,"noEmit":true},"include":["src/**/*.ts"]}\n'),
  );
  assertDrift(
    () => writeFileSync(join(root, "packages/tool/package.json"), '{"name":"@acme/tool","exports":{}}\n'),
    () => writeFileSync(join(root, "packages/tool/package.json"), '{"name":"@acme/tool","exports":{".":"./src/index.ts"}}\n'),
  );
  assertDrift(
    () => {
      unlinkSync(join(root, "apps/web/src/link.ts"));
      symlinkSync("target-b.ts", join(root, "apps/web/src/link.ts"));
    },
    () => {
      unlinkSync(join(root, "apps/web/src/link.ts"));
      symlinkSync("target-a.ts", join(root, "apps/web/src/link.ts"));
...
test("batch authority covers transitive declarations reached only from configured consumers", () => {
  ...
  writeFileSync(join(root, "apps/web/src/main.ts"), "export const main = 1;\n");
  writeFileSync(
    join(root, "apps/web/consumers/check.ts"),
    'import type { Public } from "../../vendor/feature/index.ts"; export const check: Public = { value: 1 };\n',
  );
  ...
  const config = parseConfig({
    ...
    firstPartyPackages: [{ root: "vendor/feature", name: "@acme/feature" }],
    packageManager: "bun",
    scaffoldTemplates: { packageJson: { contents: '{"name":"{package}"}\n' } },
  });
  const options = { config, configPath, rootDir: root };
  ...
  expect(inventory.entries).toContainEqual(
    expect.objectContaining({ namespace: "installed", path: "node_modules/@acme/transitive/nested.d.ts", kind: "file" }),
  );
...
test("unnamed external inputs with the same basename retain distinct inventory identities", () => {
  ...
  const uncapturedReport = canonicalizeScanReport(root, { modules: [{ source: uncaptured, dependencies: [] }], observedReads: [uncaptured] });
  expect(() => assertReportsBoundToInventory(root, inventory, { web: uncapturedReport })).toThrow(InputInventoryError);
  ...
  expect(() => assertReportsBoundToInventory(root, inventory, { web: { modules: [], observedReads: [canonicalInputPath(root, missingFirst)] } })).toThrow(
    InputInventoryError,
  );
