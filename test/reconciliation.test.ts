...
function manifest(): ExtractionManifest {
  return {
    ...
    consumers: [],
    generatedFiles: [
      { path: "packages/unit/tsconfig.json", source: "config", regenerate: "synthetic-generator", regenerateOnApply: true, expectedHash: HASH_A },
    ],
...
function proof(passed = true) {
  return { passed, checked: 1, failures: passed ? [] : ["synthetic failure"] };
}
...
function audit(overrides: Partial<AuditReport> = {}): AuditReport {
  return {
    ...
    generatedArtifacts: proof(false),
...
function record(auditedRoot = "/synthetic") {
  ...
  return compileReconciliationRecord({
    manifest: value,
    manifestBytes: `${JSON.stringify(value)}\n`,
    manifestPath: "plans/synthetic.json",
    approvalCommit: APPROVAL,
    moveCommit: MOVE,
    wiringCommit: WIRING,
    observedHead: LATER,
    observedCommitDate: "2026-02-03T04:05:06-06:00",
    audit: audit({ auditedRoot }),
    discrepancies: [{ path: "packages/unit/tsconfig.json", actual: HASH_B }],
    reason: "generator produced the reviewed solution reference set",
    approval: { subject: "chore: accept synthetic reconciliation" },
  });
}
...
describe("immutable reconciliation evidence", () => {
  ...
  test("refuses undeclared drift and structural failures", () => {
    expect(() =>
      compileReconciliationRecord({
        ...
        wiringCommit: WIRING,
        observedHead: LATER,
        observedCommitDate: "2026-01-01T00:00:00Z",
        audit: audit(),
        discrepancies: [{ path: "undeclared.txt", actual: HASH_B }],
        reason: "reason",
    ...
    expect(() =>
      compileReconciliationRecord({
        ...
        observedHead: LATER,
        observedCommitDate: "2026-01-01T00:00:00Z",
        audit: audit({ boundaryRules: proof(false) }),
        discrepancies: [{ path: "packages/unit/tsconfig.json", actual: HASH_B }],
        reason: "reason",
        approval: { subject: "approval" },
      }),
  ...
  test("record identity detects changed evidence and non-discrepancies", () => {
    ...
    expect(() =>
      compileReconciliationRecord({
        ...
        observedCommitDate: "2026-01-01T00:00:00Z",
        audit: audit(),
        discrepancies: [{ path: "packages/unit/tsconfig.json", actual: HASH_A }],
        reason: "reason",
        approval: { subject: "approval" },
      }),
    ...
    expect(() =>
      compileReconciliationRecord({
        manifest: manifest(),
        manifestBytes: JSON.stringify({ ...manifest(), planId: "different-plan" }),
        manifestPath: "plans/synthetic.json",
        approvalCommit: APPROVAL,
        moveCommit: MOVE,
        wiringCommit: WIRING,
  ...
  test("issues deterministic receipts only for passing identity-matched audits", () => {
    ...
    const input = {
      record: {
        path: "plans/reconciliation.json",
        bytes: serializeReconciliationRecord(reconciliation),
        value: reconciliation,
        approvalCommit: "6".repeat(40),
      },
      plan: reconciliation.plan,
      application: reconciliation.application,
      observedCommit: LATER,
      observedCommitDate: "2026-02-03T10:05:06Z",
      audit: passing,
      manifest: manifest(),
    };
    ...
    expect(serializeAppliedPlanReceipt(compileAppliedPlanReceipt({ ...input, audit: { ...passing, auditedRoot: "/another/checkout" } }))).toBe(
      serializeAppliedPlanReceipt(receipt),
    );
    ...
    expect(() =>
      compileAppliedPlanReceipt({ ...input, record: { ...input.record, bytes: JSON.stringify({ ...reconciliation, reason: "tampered bytes" }) } }),
    ...
    const changedProvenance = {
      ...
      provenance: {
        configDigest: HASH_A,
        policyDigest: HASH_A,
        compiler: { artifactIntegrity: HASH_B },
