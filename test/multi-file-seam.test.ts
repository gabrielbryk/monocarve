...
function write(root: string, path: string, text: string): void {
  const absolute = join(root, path); mkdirSync(dirname(absolute), { recursive: true }); writeFileSync(absolute, text);
}
...
test("finds one exact cross-file type cycle and keeps disjoint groups separate", () => {
  ...
  expect(cycle?.affectedConsumers).toEqual(expect.arrayContaining([
...
test("stale observations refuse without returning plausible candidates", () => {
  ...
  const result = planMultiFileSeams({
    rootDir: root, tsconfigPath: "tsconfig.json", sourcePaths: ["src/a.ts", "src/b.ts"], affinityForPath: () => "shared",
...
test("refuses unsafe source scope and conflicting placement for an atomic cycle", () => {
  ...
  const collided = planMultiFileSeams({ rootDir: root, tsconfigPath: "tsconfig.json", sourcePaths: ["src/a.ts", "src/b.ts"], affinityForPath: () => "shared", targetPaths });
...
test("compiles and independently simulates a reviewed cross-file type cycle as one atomic manifest", async () => {
  ...
  const config = fixtureConfig(root, { preparation: { gates: { package: [], project: [], workspace: ["true"] }, commit: { subject: "refactor: prepare cyclic types" } } });
  ...
  const members = files.map((file) => {
    ...
    return { seam, targetPath, targetModuleSpecifier: `./${name.toLowerCase()}-types.ts`, reviewedGroupIds: seam.movedGroups.map((group) => group.id), rendering: { gates: { package: [], project: [], workspace: ["true"] }, commit: { subject: "refactor: prepare cyclic types" } }, rewriteRelativeTypeImport: ({ originalSpecifier }: { originalSpecifier: string }) => ({ targetSpecifier: originalSpecifier, resolvedSourcePath: originalSpecifier === "./a" ? "apps/api/src/a.ts" : "apps/api/src/b.ts" }) };
  });
  const manifest = compileMultiFilePreparationManifest({ rootDir: root, config, baselineCommit: "HEAD", graphDigest: hashJson(multi), multiSeam: multi, candidateId: candidate.id, members });
  expect(() => compileMultiFilePreparationManifest({ rootDir: root, config, baselineCommit: "HEAD", graphDigest: hashJson(multi), multiSeam: multi, candidateId: candidate.id, members: members.slice(0, 1) })).toThrow(/exactly cover/);
