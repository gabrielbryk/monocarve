...
test("dependency and artifact explanations expose exact persisted provenance", () => {
  const operation = { kind: "write-file" as const, path: "libs/chart/package.json", contents: "{}\n", preconditionHash: "missing" as const, resultHash: hashText("{}\n"), generator: "scaffold:package-json" };
  const manifest = {
    planId: "c-example", dependencies: { runtime: { library: "1" }, dev: {}, packageReferences: [] },
    dependencyDecisions: [{ name: "library", decision: "target-runtime", sources: ["apps/web/src/chart.ts"], reasons: ["production-import"] }],
    projectedArtifacts: [{ path: operation.path, kind: "json", resultHash: operation.resultHash }], operations: [operation],
  } as unknown as ExtractionManifest;
  expect(explainDependency(manifest, "library")).toMatchObject({ declarations: [{ section: "runtime", version: "1" }], decisions: [{ sources: ["apps/web/src/chart.ts"] }] });
