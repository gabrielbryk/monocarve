...
const input = {
  sourcePath: "apps/consumer/src/model.ts",
  targetPath: "apps/consumer/src/model-types.ts",
  targetModuleSpecifier: "./model-types.js",
};
...
describe("preparation policy", () => {
  ...
  test("renders only configured repository gates and commit metadata deterministically", () => {
    const config = parseConfig({
      ...
      preparation: {
        ...
        gates: {
          workspace: ["verify {targetPath}", "check {sourcePath}"],
          project: [],
        },
      },
    });
    ...
    expect(renderPreparationPolicy(config, input)).toEqual({
      ...
      gates: {
        package: [],
        project: [],
        workspace: ["check apps/consumer/src/model.ts", "verify apps/consumer/src/model-types.ts"],
      },
    });
  });
  ...
  test("rejects unsupported placeholders and duplicate rendered gates", () => {
    const unknown = parseConfig({
      ...base,
      preparation: { commit: { subject: "chore: prepare {package}" }, gates: { workspace: ["verify"] } },
    });
  ...
  test("detects a manifest policy that drops a configured gate or changes its commit", () => {
    const config = parseConfig({
      ...base,
      preparation: { commit: { subject: "chore: prepare declarations" }, gates: { workspace: ["verify {sourcePath}"] } },
    });
    const rendered = renderPreparationPolicy(config, input);

    expect(() => assertPreparationPolicyMatches(config, input, {
      ...rendered,
      gates: { ...rendered.gates, workspace: [] },
    })).toThrow(ConfigError);
    expect(() => assertPreparationPolicyMatches(config, input, {
      ...rendered,
      commit: { subject: "chore: changed" },
