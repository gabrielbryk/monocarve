...
function workspace(lazySource = 'export const loadChart = () => import("./widgets/chart.ts");\n', subpaths = false): string {
  const root = join(scratchDirectory(), "dynamic-workspace");
  cpSync(FIXTURE, root, { recursive: true });
  rmSync(join(root, ".monocarve"), { recursive: true, force: true });
  writeFileSync(join(root, LAZY), lazySource);
  if (subpaths) {
    ...
    config.scaffoldTemplates.publicSurface = { mode: "subpaths", keyTemplate: "./{pathNoExtension}", targetTemplate: "./src/{path}" };
...
test("applies and audits a literal dynamic consumer without collapsing its lazy boundary", async () => {
  ...
  expect(manifest.consumers.find((consumer) => consumer.file === LAZY)?.specifiers).toEqual([
    { from: "./widgets/chart.ts", to: "@acme/chart/widgets/chart", donor: "apps/web/src/widgets/chart.ts" },
  ]);
  expect(manifest.expectedDynamicImportDelta).toEqual({ added: ["@acme/chart/widgets/chart"], removed: ["./widgets/chart.ts"] });
  ...
  expect(wrongSubpath.graphEvidence.dynamicImportDelta).toEqual({ added: ["@acme/chart/types"], removed: ["./widgets/chart.ts"] });
  ...
  write(root, LAZY, 'import * as chart from "@acme/chart/widgets/chart";\nexport const loadChart = async () => chart;\n');
  const madeStatic = auditPlanSync({ config, rootDir: root, manifest, skipCompileProof: true });
  expect(madeStatic.graphEvidence.dynamicImportDelta).toEqual({ added: [], removed: ["./widgets/chart.ts"] });
...
test("keeps the default barrel manifest free of subpath-only fields and byte-deterministic", async () => {
  ...
  const input = { config, rootDir: root, graph, candidate: candidate!, baselineCommit: graph.commit!, packageName: "@acme/chart" };
...
test("rewrites a proven production consumer edge alongside an unrelated computed import", async () => {
  ...
  const testCandidate = buildPortfolio({ config: testConfig, graph: testGraph }).candidates.find((entry) => entry.files.includes(CHART));
  ...
  const productionCandidate = buildPortfolio({ config: productionConfig, graph: productionGraph }).candidates.find((entry) => entry.files.includes(CHART));
  ...
  expect(findConsumers(productionContext, [CHART], "@acme/chart")).toContainEqual(
    expect.objectContaining({ file: LAZY, rewrites: [{ from: "./widgets/chart.ts", to: "@acme/chart", donor: CHART }], donors: [CHART] }),
  );
...
test("keeps donor identity when one consumer mixes package-root and subpath destinations", () => {
  ...
  const root = workspace(
    ['import { renderChart } from "./widgets/chart.ts";', 'import { format } from "./widgets/format.ts";', "void renderChart; void format;", ""].join("\n"),
  );
  ...
  const context = new WorkspaceContext(config, root);
  const consumers = findConsumers(context, [CHART, format], "@acme/extracted", new Map([[CHART, "@acme/extracted/widgets/chart"]]));
...
test("simulates a declared JSON runtime registry rewrite before external package compilation", async () => {
  ...
  raw.runtimeModuleRegistries = [
    { file: "apps/web/scripts/route-registry.json", pointer: "/domains/*/module", resolveFrom: "apps/web/src", stripPrefix: "./" },
  ];
  raw.postJournalPreparers = [
    {
      ...
      emittedModuleSpecifiers: [{ source: "apps/web/scripts/route-stubs.ts", resolutionBase: "apps/web/src/routes.ts" }],
    },
  ];
  ...
  writeFileSync(
    lockfilePath,
    readFileSync(lockfilePath, "utf8").replace(
  ...
  write(
    root,
    "apps/web/scripts/route-registry.json",
    `${JSON.stringify({ domains: [{ id: "chart", module: "./widgets/chart.ts", export: "renderChart" }] }, null, 2)}\n`,
  );
  write(
    root,
    "apps/web/scripts/route-introspection.ts",
    [
  ...
  write(
    root,
    "apps/web/scripts/route-stubs.ts",
    [
  ...
  write(
    ...
    'import { renderChart as governedChart } from "./widgets/chart.ts";\nvoid governedChart;\nexport { renderChart } from "./widgets/chart.ts";\n',
  );
  fixtureGit(root, "add", "-A");
  ...
  expect(candidate).toBeDefined();
  const manifest = buildPlanSync({ config, rootDir: root, graph, candidate: candidate!, baselineCommit: graph.commit!, packageName: "@acme/chart" });
  const registryRewrite = manifest.operations.find(
    (operation) => operation.kind === "rewrite-path-reference" && operation.file === "apps/web/scripts/route-registry.json",
  );
  ...
  const templateRewrite = manifest.operations.find(
    (operation) => operation.kind === "rewrite-path-reference" && operation.file === "apps/web/scripts/route-stubs.ts",
  );
  ...
  expect(templateRewrite.rewrites).toEqual([
    expect.objectContaining({
      from: "./widgets/chart.ts",
      to: "../../../libs/chart/src/widgets/chart.ts",
      donor: CHART,
  ...
  const tampered = {
    ...manifest,
    operations: manifest.operations.map((operation) =>
      operation === registryRewrite ? { ...operation, rewrites: operation.rewrites.map(({ strippedPrefix: _removed, ...rewrite }) => rewrite) } : operation,
    ),
  };
  ...
  expect(
    manifest.operations.some(
      (operation) => (operation.kind === "rewrite-import" || operation.kind === "rewrite-fs-reference") && operation.file === "apps/web/src/routes.ts",
    ),
  ).toBe(false);
  ...
  expect(regenerateArtifacts({ config, treeRoot: root, manifest: missingGeneratorRecord }).failure).toContain(
    "configured post-journal preparer(s) missing from plan: route-stubs",
  );
  ...
  const staleManifest = buildPlanSync({
    config: staleConfig,
    rootDir: root,
    graph,
    candidate: candidate!,
  ...
  expect(simulation.regeneration?.artifacts).toContainEqual(expect.objectContaining({ path: "apps/web/src/routes.ts", changed: true, exitCode: 0 }));
}, 300_000);

test("refuses an emitted module specifier that ambiguously identifies multiple moves", () => {
  expect(() =>
    scanEmittedModuleSpecifiers(
...
test("continues to reject a genuinely computed module reference inside moved production code", async () => {
  const root = workspace();
  write(root, CHART, "const modulePath = `./renderers/${name}.ts`;\nexport const renderChart = () => import(modulePath);\n");
  const { config } = await loadConfig({ cwd: root });
  const graph = await scanDependencyGraph({ config, rootDir: root, noCache: true });
  const candidate = buildPortfolio({ config, graph }).candidates.find((entry) => entry.files.includes(CHART));

  expect(candidate?.rejectionReasons).toContainEqual(expect.objectContaining({ code: "unsupported-module-reference", edges: expect.arrayContaining([CHART]) }));
