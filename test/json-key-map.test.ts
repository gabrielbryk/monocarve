...
describe("JSON path-key migration filter", () => {
  ...
  test("refuses an occupied destination", () => {
    expect(() =>
      relocateJsonObjectKeys({ artifact: "baseline.json", contents: '{"files":{"a.ts":1,"b.ts":2}}', moves: [{ source: "a.ts", target: "b.ts" }] }, "/files"),
    ).toThrow("destination key already exists");
  });
});
