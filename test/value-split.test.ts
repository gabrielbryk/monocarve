...
function setup(source: string) {
    ...
    postJournalPreparers: [
      {
        ...
        outputs: ["apps/api/ledger.txt", "apps/api/stable-ledger.txt"],
  ...
  const rendering = {
    commit: { subject: "refactor: split normalize value" },
    gates: { package: [], project: [], workspace: ["grep -q generated apps/api/ledger.txt"] },
  };
...
describe("value split", () => {
  test("moves exact standalone value bytes and retains a compatibility export", async () => {
    ...
    expect(
      auditPreparationSync({
        ...
        rootDir: fixture.root,
        manifest,
        freshGraph: { commit: fixture.baseline.commit, digest: manifest.graphDigest },
      }).passed,
    ).toBe(true);
  });
  ...
  test("refuses a helper dependency that retained code still uses", () => {
    ...
    ).toThrow(/still used by retained declaration.*trim/);
  });
  ...
  test("refuses effectful runtime initialization whose evaluation order would move", () => {
    ...
    ).toThrow(/top-level evaluation effects/);
  });
});
