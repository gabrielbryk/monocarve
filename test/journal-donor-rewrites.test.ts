...
function rewriteResult(root: string, rewrites: readonly Required<ImportRewrite>[]): string {
  return rewrites.reduce(
    (source, rewrite) =>
      rewriteResolvedImportSpecifier(source, join(root, CONSUMER), join(root, rewrite.donor), rewrite.to, root),
    CONSUMER_TEXT,
  );
}
...
function manifest(root: string, operations: readonly PlanOperation[]): ExtractionManifest {
  ...
  return {
    ...
    application: "api",
    target: {
      packageName: "@acme/values",
      packageRoot: "libs/values",
    ...
    consumers: [],
    generatedFiles: [],
    changedFiles: [...new Set(operations.flatMap((operation) => {
    ...
    metrics: {
      movedFiles: 2,
      movedLines: 2,
      applicationLinesBefore: 5,
      applicationLinesAfter: 3,
      consumers: 1,
    },
    commits: {
...
describe("donor-specific journal rewrites", () => {
  ...
  test("refuses mixed donor-specific and legacy rewrites without changing the consumer", async () => {
    ...
    await expect(executeJournal({ config, treeRoot: root, manifest: manifest(root, [operation]) })).rejects.toThrow(
      "mixes donor-specific and legacy rewrites",
    );
  ...
  test("restores a donor-specific rewrite and earlier writes when a later operation fails", async () => {
    ...
    const operations: PlanOperation[] = [
      ...
      {
        kind: "write-file",
        path: "libs/values/generated.txt",
        contents: "generated\n",
        preconditionHash: "missing",
        resultHash: hashText("generated\n"),
      },
      {
        kind: "write-file",
        path: "libs/values/failure.txt",
        contents: "actual\n",
        preconditionHash: "missing",
        resultHash: hashText("different\n"),
      },
    ];

    await expect(executeJournal({ config, treeRoot: root, manifest: manifest(root, operations) })).rejects.toThrow(
      "hash mismatch",
    );
    expect(read(root, CONSUMER)).toBe(CONSUMER_TEXT);
