...
function manifest(root: string): ExtractionManifest {
  return {
    planId: "p-test", baselineCommit: fixtureGit(root, "rev-parse", "HEAD"),
...
describe("durable apply transaction state", () => {
  test("serializes committing applies and reports the live owner", () => {
    ...
    expect(() => beginApplyTransaction(root, manifest(root), ".plans/p-test.json"))
      .toThrow("apply transaction p-test is simulating");
  ...
  test("releases only a stopped matching owner and guides move-boundary resume", () => {
    ...
    const stale: ApplyTransactionState = {
      schema: "apply-transaction-v1", planId: "p-test", manifestPath: ".plans/p-test.json",
      baselineCommit: startHead, startHead,
      ownerPid: 2_000_000_000, ownerToken: "stopped-owner", phase: "applying",
    };
    ...
    expect(recoverApplyTransaction(root, manifest(root)).next).toEqual([
      "monocarve", "apply", "--plan", ".plans/p-test.json", "--commit", "--resume",
    ]);
    expect(applyTransactionStatus(root)).toEqual({ active: false, ownerAlive: false });
