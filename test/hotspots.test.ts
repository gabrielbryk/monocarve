...
function candidate(id: string, files: string[], lineCount: number, domains: string[]): PortfolioCandidate {
  return {
    ...
    rewriteEscapes: [],
    recommendation: {
      status: domains.length > 1 ? "discouraged" : "recommended",
      cohesion: domains.length > 1 ? "low" : "high",
      reasons: [],
      requiresExplicitPackageName: false,
      highInboundModules: [],
      targetOptions: [],
    },
  };
...
test("ranks a high-inbound cross-domain module above leaf modules", () => {
  ...
  const graph = {
    paths,
    nodes: new Map(paths.map((path) => [path, { path, zone: "application", owner: "apps/web", application: "web", domain: "web", lineCount: 10 }])),
    incoming: new Map([[paths[0], Array.from({ length: 9 }, (_, index) => `consumer-${index}`)]]),
    outgoing: new Map([[paths[0], [paths[1], paths[2]]]]),
    edges: [],
    testImporters: new Map(),
    workspace: { owners: [], packageNames: new Map() },
    rootDir: "/repo",
  ...
  const portfolio = {
    ...
    candidates,
    selected: [],
    equivalenceGroups: candidates.map((item) => ({
      id: `g-${item.id}`,
      representativeId: item.id,
      candidateIds: [item.id],
      sharedFileCount: item.files.length,
      similarity: 1,
    })),
  } satisfies Portfolio;
...
test("breaks equal-pressure hotspot ties by code unit", () => {
  ...
  expect(analyzeCouplingHotspots(graph, portfolio).map((entry) => entry.path)).toEqual(["apps/web/src/Z.ts", "apps/web/src/a.ts"]);
});
