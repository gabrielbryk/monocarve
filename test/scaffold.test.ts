...
function existingPackageOperations(manifest: Record<string, unknown>, publicModule: PublicModule = PUBLIC_MODULE) {
  const root = fixtureRepo({ "libs/new-package/package.json": `${JSON.stringify(manifest, null, 2)}\n` });
...
function writtenPackageJson(operations: ReturnType<typeof existingPackageOperations>): Record<string, unknown> {
  const operation = operations.find((entry) => entry.kind === "write-file" && entry.path === "libs/new-package/package.json");
...
describe("package scaffold", () => {
  ...
  test("keeps a subpaths-only package entrypoint inert", () => {
    ...
    expect(operations).toContainEqual(
      expect.objectContaining({ kind: "write-file", path: "libs/browser-ui/src/index.ts", contents: "", generator: "scaffold:entrypoint" }),
    );
  });
  ...
  test("converts a bare root export before adding module subpaths", () => {
    const manifest = writtenPackageJson(
      existingPackageOperations({ name: "@acme/new-package", exports: "./src/index.ts", dependencies: {}, devDependencies: {} }),
    );

    expect(manifest.exports).toEqual({ ".": "./src/index.ts", "./widgets/chart": "./src/widgets/chart.ts" });
  });

  test("preserves root conditions and unrelated wildcard exports", () => {
    const rootConditions = { types: "./src/index.ts", import: "./src/index.ts", default: "./src/index.ts" };
    const conditional = writtenPackageJson(
      existingPackageOperations({ name: "@acme/new-package", exports: rootConditions, dependencies: {}, devDependencies: {} }),
    );
    expect(conditional.exports).toEqual({ ".": rootConditions, "./widgets/chart": "./src/widgets/chart.ts" });

    const mapped = writtenPackageJson(
      existingPackageOperations({
        name: "@acme/new-package",
        exports: { ".": rootConditions, "./legacy/*": "./src/legacy/*.ts" },
        dependencies: {},
        devDependencies: {},
      }),
    );
    expect(mapped.exports).toEqual({ ".": rootConditions, "./legacy/*": "./src/legacy/*.ts", "./widgets/chart": "./src/widgets/chart.ts" });
  });

  test("accepts an identical subpath idempotently and refuses a conflict", () => {
    const manifest = {
      name: "@acme/new-package",
      exports: { ".": "./src/index.ts", "./widgets/chart": "./src/widgets/chart.ts" },
      dependencies: {},
      devDependencies: {},
    };
    expect(existingPackageOperations(manifest)).toEqual([]);

    expect(() => existingPackageOperations({ ...manifest, exports: { ...manifest.exports, "./widgets/chart": "./src/other.ts" } })).toThrow(
      new PlanningError('libs/new-package/package.json export ./widgets/chart already targets "./src/other.ts", not "./src/widgets/chart.ts"'),
    );
  });
  ...
  test("refuses an existing exports object that mixes conditions and subpaths", () => {
    ...
    ).toThrow(new PlanningError("libs/new-package/package.json exports cannot mix package subpaths and root conditions"));
  });
  ...
  test("registers through an adapter-owned project registry path", () => {
    ...
    const taskRunner: TaskRunnerAdapter = {
      ...
      registerProject: (text, packageRoot, projectId) => ({ kind: "changed", contents: `${text}register ${projectId} ${packageRoot}\n` }),
      wrapGateCommand: (command) => ["sh", "-c", command],
    };
    ...
    const registration = operations.find((operation) => operation.kind === "write-file" && operation.generator === "scaffold:project-registration");
  ...
  test("refuses an adapter precondition it cannot satisfy", () => {
    ...
    const taskRunner: TaskRunnerAdapter = {
      ...
      projectIdFor: (packageName) => packageName,
      projectIdOf: (_rootDir, packageRoot) => packageRoot,
      registerProject: () => ({ kind: "unmet-precondition", reason: "the fixture registry cannot accept new projects" }),
      wrapGateCommand: (command) => ["sh", "-c", command],
    };
  ...
  test("refuses to scaffold without the required lockfile", () => {
    const root = fixtureRepo({ ".gitignore": "node_modules\n", "pnpm-workspace.yaml": "packages:\n  - libs/*\n" });
    const config = fixtureConfig(root);
    const context = new WorkspaceContext(config, root);
