...
function candidate(id: string, files: readonly string[], score: number): PortfolioCandidate {
  return {
    ...
    assets: [],
    sccs: [],
    seed: { id: `scc-${id}`, members: [files[0]!] },
...
describe("architectural portfolio evidence", () => {
  test("groups near-identical closures but leaves a distinct idea separate", () => {
    const groups = groupEquivalentCandidates(
      [candidate("a", ["a.ts", "b.ts", "c.ts"], 3), candidate("b", ["a.ts", "b.ts", "c.ts"], 4), candidate("c", ["other.ts"], 10)],
      0.95,
    );
    expect(groups).toHaveLength(2);
    expect(groups.find((group) => group.candidateIds.includes("a"))).toMatchObject({ representativeId: "b", candidateIds: ["a", "b"], similarity: 1 });
  });
  ...
  test("a pure workspace re-export is detectable, while one own declaration makes the proof fail", () => {
    ...
    expect(describePureReexport(parse('export { PublicHelp } from "@acme/page-help/help";'), ["@acme/page-help"])).toEqual({
      packageName: "@acme/page-help",
      specifier: "@acme/page-help/help",
      symbols: ["PublicHelp"],
    });
    expect(describePureReexport(parse('const local = 1; export { PublicHelp } from "@acme/page-help/help";'), ["@acme/page-help"])).toBeUndefined();
  });
  ...
  test("an existing suggested package requires extension review instead of being called a compatible creation", () => {
    const graph = { workspace: { packageNames: new Map([["@acme/existing", "libs/existing"]]) } } as unknown as DependencyGraph;
