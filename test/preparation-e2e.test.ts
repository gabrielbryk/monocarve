...
describe("type-only seam preparation", () => {
  test("plans, simulates, applies, audits, rescans, then permits its dependent extraction", async () => {
    ...
    const safety = classifyTypeOnlyExtraction({
      sourceText: read(fixture.root, PREPARATION_DONOR), graph: analysis.source, groupId: group.id, selectedDeclarationIds: group.declarationIds,
    });
    ...
    const preparationChild = {
      id: "prepare-contract-types", pairId: "contract-types", kind: "preparation" as const,
      planId: manifest.planId, baselineCommit: manifest.baseline.commit,
    };
    const ledger = appendCampaignChild(createCampaignLedger({
      campaignId: "contract-seam", objective: "prepare a contract seam", stopConditions: [], baselineCommit: manifest.baseline.commit, initialGraph: before,
    }), preparationChild);
    ...
    const result = await advanceCampaign({
      ...
      compileNext: ({ headCommit }) => ({
        id: "extract-contract-boundary", pairId: "contract-types", kind: "extraction",
        planId: "reviewed-dependent-extraction", baselineCommit: headCommit,
      }),
    });
  ...
  test("audit rejects a tampered preparation target and campaign advance rejects auditless preparation", async () => {
    ...
    expect(auditPreparationSync({
      config: fixture.config,
      rootDir: fixture.root,
      manifest,
      freshGraph: { commit: manifest.baseline.commit, digest: manifest.graphDigest },
    }).byteReplay.passed).toBe(false);
    const before = graphSnapshot(analysis);
    const planned = appendCampaignChild(createCampaignLedger({
      campaignId: "audit-required", objective: "prove audited ordering", stopConditions: [], baselineCommit: manifest.baseline.commit, initialGraph: before,
    }), {
      id: "prepare-contract-types", pairId: "contract-types", kind: "preparation",
      planId: manifest.planId, baselineCommit: manifest.baseline.commit,
    });
    await expect(advanceCampaign({
      campaign: planned, headCommit: () => manifest.baseline.commit, rescan: () => before,
      compileNext: () => ({
        id: "extract-contract-boundary", pairId: "contract-types", kind: "extraction",
        planId: "blocked", baselineCommit: manifest.baseline.commit,
      }),
  ...
  test("commits and independently audits a relocated relative inline import type", async () => {
    ...
    const applied = await applyPreparation({
      config: fixture.config, rootDir: fixture.root, manifest, manifestPath: fixture.planPath,
      commit: true, baselineGraphScanner: async ({ baselineCommit }) => ({ commit: baselineCommit, digest: manifest.graphDigest }),
    });
  ...
  test("commits and independently audits exported ambient types with empty JavaScript output", async () => {
    const source = [
      "export declare type AmbientContract = { readonly name: string };",
      "export const runtimeMarker = 1;",
      "",
    ].join("\n");
...
function preparationCaseConfig(root: string) {
  return fixtureConfig(root, {
    preparation: {
      ...
      commit: { subject: "refactor: prepare compact e2e" },
    },
  });
}
...
function compileCase(
  ...
  const seam = planSeam({
    analysis,
    sourceText: read(fixture.root, fixture.donorPath),
    candidateId: candidate.id,
