...
function analyzeFile(path: string, src: string) {
  ...
  const classes: {
    ...
    methods: { name: string; fields: Set<string>; isCtor: boolean }[];
    ctorParams: number;
  }[] = [];
  const sig: Signals = {
  ...
  function fnMetrics(body: any, nameHint: string): Fn {
    ...
    (function walk(n: any, depth: number) {
      ...
      if (t === 'SwitchStatement' && Array.isArray(n.cases))
        cyclo += n.cases.length;
      if (
        t === 'LogicalExpression' &&
    ...
    const loc =
      body?.start != null && body?.end != null
        ? lineAt(body.end) - lineAt(body.start) + 1
        : 0;
    return {
      cyclo,
      maxNest,
  ...
  (function top(n: any) {
    ...
    if (t === 'MemberExpression' && n.object?.type === 'Identifier') {
      ...
      if (o === 'z') sig.zCalls++;
      if (o === 'Effect' && (prop === 'gen' || prop === 'Service'))
        sig.hasEffect = true;
      if (o === 'Context' && prop === 'Tag') sig.hasEffect = true;
    }
    if (Array.isArray(n.decorators))
      for (const d of decoratorNames(n.decorators))
        if (RESOLVER_DECORATORS.has(d)) sig.hasResolverDecorator = true;
    ...
    if (t === 'ClassDeclaration' || t === 'ClassExpression') {
      if (n.superClass?.type === 'Identifier')
        sig.superClasses.add(n.superClass.name);
      const cls = {
        deps: new Set<string>(),
  ...
  const abs =
    171 -
    5.2 * Math.log(vocab || 1) -
    0.23 * fileCyclo -
    16.2 * Math.log(factor);
  ...
  if (cls) {
    ...
    const ubiq = new Set(
      [...freq]
        .filter(
          ([, c]) => c >= Math.max(3, Math.ceil(cls.methods.length * 0.4)),
        )
        .map(([f]) => f),
    );
    const spec = cls.methods.map(
      (m) =>
        new Set(
          [...m.fields].filter((f) => !methodNames.has(f) && !ubiq.has(f)),
        ),
    );
    ...
    const find = (x: number): number =>
      parent.get(x) === x
        ? x
        : (parent.set(x, find(parent.get(x)!)), parent.get(x)!);
    for (let a = 0; a < idx.length; a++)
      for (let b = a + 1; b < idx.length; b++) {
        const i = idx[a],
          j = idx[b];
        if ([...spec[i]].some((d) => spec[j].has(d)))
  ...
  const avgCyclo = cyclos.length
    ? cyclos.reduce((a, b) => a + b, 0) / cyclos.length
    : 0;
...
for (const target of targets) {
  ...
  if (statSync(abs).isFile()) files.push(abs);
    ...
    files.push(
      ...execSync(
        `find "${abs}" -name '*.ts' -o -name '*.tsx' 2>/dev/null | grep -vE '${EXCLUDE}'`,
        { maxBuffer: 1 << 28 },
      )
        .toString()
        .trim()
...
for (const m of PCT_METRICS)
  repoVals[m] = results.map((r) => r[m] as number).sort((a, b) => a - b);
...
for (const [arch, list] of byArch) {
  ...
  for (const m of PCT_METRICS)
    rec[m] = list.map((r) => r[m] as number).sort((a, b) => a - b);
...
function diagnoseLever(r: Result): { lever: string; leverReason: string } {
  ...
  if (r.responsibilities >= 2 && p.responsibilities > 60)
    return {
      lever: 'split-service',
      leverReason: `${r.responsibilities} responsibility clusters (p${p.responsibilities})`,
    };
  if (top.name === 'maxCognitive' && p.maxCognitive > 75)
    return {
      lever: 'extract-method',
      leverReason: `worst method cognitive ${r.maxCognitive} (p${p.maxCognitive})`,
    };
  if (top.name === 'maxNest' && p.maxNest > 60)
    return {
      lever: 'flatten-nesting',
      leverReason: `nesting depth ${r.maxNest} (p${p.maxNest})`,
    };
  if (top.name === 'fanOut' && p.fanOut > 60)
    return {
      lever: 'reduce-coupling',
      leverReason: `fans out to ${r.fanOut} collaborators (p${p.fanOut})`,
    };
  if (r.ctorParams > 5)
    return {
      lever: 'deps-bag',
      leverReason: `constructor takes ${r.ctorParams} params`,
    };
  if (r.archetype === 'react-component' && p.structuralScore > 60)
    return {
      lever: 'decompose-component',
      leverReason: `heavy component (struct p${p.structuralScore})`,
    };
  if (r.archetype === 'react-hook' && p.structuralScore > 60)
    return {
      lever: 'extract-hook',
      leverReason: `heavy hook (struct p${p.structuralScore})`,
    };
  if (top.pct <= 60) return { lever: 'none', leverReason: 'no term above p60' };
  return {
    lever: 'none',
    leverReason: `${top.name} at p${top.pct}, below action threshold`,
  };
}
...
for (const r of results) {
  ...
  for (const m of PCT_METRICS)
    r.percentiles[m] = pctRank(src[m], r[m] as number);
