...
export function lineIndexer(src: string) {
  const nl: number[] = [0];
  for (let i = 0; i < src.length; i++) if (src.charCodeAt(i) === 10) nl.push(i + 1);
  return (off: number) => {
    let lo = 0,
      hi = nl.length - 1;
    while (lo < hi) {
...
export type Fn = { cyclo: number; maxNest: number; loc: number; name: string; cog: number };
...
export function cognitiveOf(body: any, fnName: string): number {
  let score = 0;
  const self = fnName && fnName !== "(anon)" && fnName !== "(computed)" ? fnName : null;

  function selfCallName(callee: any): string | null {
    if (!callee) return null;
    if (callee.type === "Identifier") return callee.name;
    if (callee.type === "MemberExpression" && callee.object?.type === "ThisExpression" && callee.property?.type === "Identifier") return callee.property.name;
    return null;
  }
...
export const RESOLVER_DECORATORS = new Set(["Resolver", "Query", "Mutation", "ResolveField", "Subscription"]);
...
export function classifyArchetype(path: string, sig: Signals): string {
  ...
  if (sig.superClasses.has("TypedCommandBase") || sig.superClasses.has("BaseCommandHandler") || (p.includes("/commands/") && isHandlerName))
    return "command-handler";
  if (sig.superClasses.has("BaseQueryHandler") || (p.includes("/queries/") && isHandlerName)) return "query-handler";
  if (p.includes(".entity.") || (p.includes("/models/") && sig.hasStaticCreate && sig.hasPrivateCtor)) return "entity";
  if (p.includes(".vo.")) return "value-object";
  if (p.includes(".mapper.") || stem.endsWith("Mapper") || stem.endsWith("mapper")) return "mapper";
  if (p.includes(".repository.") || p.includes("prisma-")) return "repository";
  if (/^use[A-Z]/.test(stem) || /\buse[A-Z]/.test(stem) || /^use-[a-z0-9]+(?:-[a-z0-9]+)*$/.test(stem)) return "react-hook";
  if (p.endsWith(".tsx") && sig.hasJSX) return "react-component";
  if (p.includes(".schema.") || p.includes(".schemas.") || sig.zCalls >= 5) return "zod-schema";
