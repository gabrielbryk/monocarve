...
try {
  ...
  writeFileSync(join(workspace, "apps/web/tsconfig.json"), '{"compilerOptions":{"module":"esnext","moduleResolution":"bundler","allowImportingTsExtensions":true,"noEmit":true},"include":["src/**/*.ts"]}\n');
  writeFileSync(join(workspace, "package.json"), '{"private":true,"workspaces":[]}\n');
  writeFileSync(join(workspace, "bun.lock"), "{}\n");
  writeFileSync(join(workspace, "monocarve.config.json"), `${JSON.stringify({
  ...
  const result = Bun.spawnSync([binary, "--cwd", workspace, "assess", "--app", "web", "--evidence-dir", "evidence", "--json"], { cwd: scratch, stdout: "pipe", stderr: "pipe" });
  if (result.exitCode !== 0 || !existsSync(join(workspace, "evidence/manifest.json"))) {
    throw new Error(`relocated standalone assessment failed (${result.exitCode}): ${new TextDecoder().decode(result.stderr)} ${new TextDecoder().decode(result.stdout)}`);
  }
  const manifest = JSON.parse(readFileSync(join(workspace, "evidence/manifest.json"), "utf8")) as { baseline: { runtime: { dependencies: Record<string, string> } } };
