...
| `apply-recover`     | `apply-recover --plan <path> [--force-corrupt-lock]` — refuse a live or unverifiable owner; restore a stopped owner interrupted mid-journal from its durable checkpoint (HEAD, index, and journal paths, verified), release the lock, then print the verified apply/resume argv. `--force-corrupt-lock` is accepted only for an unparseable lock and asserts no apply is running. |
