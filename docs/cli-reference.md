...
| `evacuate` | `evacuate --app <name> --source <file\|directory\|glob> [--source <...>] --package-name <name> [--package-root <path>] [--verify-lockfile] [--out <path>] [--write] [--json]` — bounded domain analysis and ordinary immutable-plan compilation. Read-only by default; `--write` exclusively creates one manifest only after eligibility passes and every boundary cut has a configured remedy. |
