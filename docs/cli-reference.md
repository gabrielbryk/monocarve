...
| `evacuate` | `evacuate --app <name> --source <file\|directory\|glob> [--source <...>] --package-name <name> [--json]` — read-only bounded domain analysis with SCC peers, retained composition, unselected dependencies, boundary cuts, and ordinary planning blockers; never writes a manifest or workspace file. |
