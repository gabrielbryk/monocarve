...
export function analyzeCapabilityPartitions(input: {
  ...
  const file = program.getSourceFile(absolute) ?? program.getSourceFiles().find((entry) => entry.fileName.replaceAll("\\", "/") === absolute.replaceAll("\\", "/"));
  ...
  const declaration = file.statements.find((statement): statement is ts.InterfaceDeclaration =>
    ts.isInterfaceDeclaration(statement) && statement.name.text === input.interfaceName
  );
  ...
  const partitions = [...grouped.values()].map((group) => ({
    affinities: group.affinities,
    properties: group.properties.sort(byCodeUnit),
    declarations: [...group.declarations].sort(byCodeUnit),
  })).sort((left, right) => byCodeUnit(left.affinities.join("/"), right.affinities.join("/")) || byCodeUnit(left.properties.join("/"), right.properties.join("/")));
  return {
...
function topLevelName(node: ts.Node): string | undefined {
  ...
  if (ts.isFunctionDeclaration(current) || ts.isClassDeclaration(current) || ts.isInterfaceDeclaration(current) || ts.isTypeAliasDeclaration(current)) return current.name?.text;
  if (ts.isVariableStatement(current)) return current.declarationList.declarations.flatMap((entry) => ts.isIdentifier(entry.name) ? [entry.name.text] : [])[0];
  return undefined;
}
