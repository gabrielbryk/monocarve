...
import { analyzeWorkspaceSymbolsWithProgram, createWorkspaceSymbolProgram, SymbolAnalysisError, WorkspaceProgramError, type ProgramCompletenessDiagnostic, type WorkspaceSymbolAnalysis, type WorkspaceSymbolProgram } from "../symbols/index.ts";
...
export interface DeclarationBatchAggregate {
  ...
  readonly selection: { readonly mode: "files" | "hotspots"; readonly requested: number; readonly selected: number; readonly deduplicated: number; readonly reason?: string };
...
export class BatchAnalysisError extends MonocarveError {
  ...
  constructor(readonly aggregate: Omit<DeclarationBatchAggregate, "schemaVersion" | "baseline">, message: string) { super(`SPLIT_ANALYSIS_INCOMPLETE: ${message}`); }
}
...
export function analyzeDeclarationBatch(snapshot: AssessmentSnapshot, selection: {
  readonly mode: "files"; readonly paths: readonly string[];
} | { readonly mode: "hotspots"; readonly count: number }): DeclarationBatchResult {
  snapshot.verify();
  const { paths, record: selectionRecord } = resolveSelection(snapshot, selection);
  validateTargets(snapshot, paths, selectionRecord);
  if (paths.length === 0) return { aggregate: { schemaVersion: 1, baseline: snapshot.baseline, application: snapshot.application, selection: selectionRecord, diagnostics: [], completed: [], failed: [], entries: [] }, reports: {} };
  const workspace = createBatchProgram(snapshot, paths, selectionRecord);
  const diagnostics = [...workspace.diagnostics, ...unsupportedProgramRelationships(workspace)]
    .sort((left, right) => byCodeUnit(left.path ?? "", right.path ?? "") || (left.start ?? -1) - (right.start ?? -1) || left.code - right.code || byCodeUnit(left.message, right.message));
  const errors = diagnostics.filter((entry) => entry.category === "error");
  if (errors.length > 0) throw new BatchAnalysisError({ application: snapshot.application, selection: selectionRecord, diagnostics, completed: [], failed: paths, entries: [] }, `the selected application program has ${errors.length} error diagnostic(s)`);
  ...
  for (const path of paths) {
    ...
    try {
      ...
      throw new BatchAnalysisError({ application: snapshot.application, selection: selectionRecord, diagnostics, completed: entries.map((entry) => entry.sourcePath), failed, entries }, detail);
    }
  }
  snapshot.verify();
  return { aggregate: { schemaVersion: 1, baseline: snapshot.baseline, application: snapshot.application, selection: selectionRecord, diagnostics, completed: entries.map((entry) => entry.sourcePath), failed: [], entries }, reports };
}

type BatchSelectionRecord = DeclarationBatchAggregate["selection"];

function resolveSelection(snapshot: AssessmentSnapshot, selection: { readonly mode: "files"; readonly paths: readonly string[] } | { readonly mode: "hotspots"; readonly count: number }): { readonly paths: string[]; readonly record: BatchSelectionRecord } {
  const raw = selection.mode === "files" ? selection.paths.map((path) => relativeWorkspacePath(snapshot.rootDir, path)) : selectHotspotTargets(snapshot, selection.count);
  const paths = [...new Set(raw)].sort(byCodeUnit);
  return { paths, record: {
    mode: selection.mode, requested: selection.mode === "files" ? raw.length : selection.count,
    selected: paths.length, deduplicated: raw.length - paths.length,
...
function validateTargets(snapshot: AssessmentSnapshot, paths: readonly string[], selection: BatchSelectionRecord): void {
  for (const path of paths) {
    ...
    if (!node || node.zone !== "application" || node.application !== snapshot.application || node.isDeclaration || node.isAsset || node.isTest) {
      throw new BatchAnalysisError({ application: snapshot.application, selection, diagnostics: [], completed: [], failed: [path], entries: [] }, `${path} is not an analyzable production source in ${snapshot.application}`);
    }
  }
}
...
function createBatchProgram(snapshot: AssessmentSnapshot, paths: readonly string[], selection: BatchSelectionRecord): WorkspaceSymbolProgram {
  try {
    ...
    const workspace = createWorkspaceSymbolProgram(snapshot.rootDir, application.tsconfig, [
      ...application.consumerRoots,
      ...snapshot.config.firstPartyRoots,
      ...snapshot.config.firstPartyPackages.map((pkg) => pkg.root),
    ], snapshot.readTypeScriptInput);
    snapshot.verify();
    return workspace;
  } catch (error) {
    const diagnostics = error instanceof WorkspaceProgramError ? [...error.completenessDiagnostics]
      : error instanceof SymbolAnalysisError ? error.diagnostics.map((entry) => ({ phase: "semantic" as const, code: entry.code, category: entry.category, message: entry.message }))
      : [{ phase: "configuration" as const, code: 0, category: "error" as const, message: error instanceof Error ? error.message : String(error) }];
    throw new BatchAnalysisError({ application: snapshot.application, selection, diagnostics, completed: [], failed: paths, entries: [] }, error instanceof Error ? error.message : String(error));
  }
}

function analyzeTarget(snapshot: AssessmentSnapshot, workspace: WorkspaceSymbolProgram, path: string): { readonly report: WorkspaceSymbolAnalysis; readonly entry: DeclarationBatchEntry } {
  ...
  return { report, entry: {
    sourcePath: path, reportPath, sourceHash: report.source.sourceHash,
    declarationCount: report.source.declarations.length, splitCandidateCount: report.splitCandidates.length,
    ...
    cycleCount: report.source.components.filter((entry) => entry.cyclic).length, status: "complete",
  } };
}
 ...
 */
function unsupportedProgramRelationships(workspace: { readonly rootDir: string; readonly program: import("typescript").Program }): ProgramCompletenessDiagnostic[] {
  ...
  for (const file of workspace.program.getSourceFiles()) {
    ...
    const visit = (node: ts.Node): void => {
      ...
      if ((dynamicImport || requireCall) && !ts.isStringLiteralLike(node.arguments[0]!)) {
        diagnostics.push({
          phase: "semantic", code: 0, category: "error",
          message: `unsupported computed module relationship (${dynamicImport ? "dynamic-import" : "require"})`,
          path: sourcePath, start: node.getStart(file), length: node.end - node.getStart(file),
        });
      }
      ts.forEachChild(node, visit);
    };
