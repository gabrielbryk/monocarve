...
import { analyzeCouplingHotspots, blockedCandidates, buildPortfolio, type CouplingHotspot, type Portfolio, type PortfolioCandidate } from "../portfolio/index.ts";
...
export interface NormalizedGraphFacts {
  readonly firstPartyModules: number; readonly applicationModules: number; readonly productionLines: number;
  readonly edges: number; readonly sccs: number; readonly cyclicSccs: number; readonly maximumLayer: number;
  readonly dynamicImports: number; readonly unresolvedImports: number;
}

export function normalizedGraphFacts(config: MonocarveConfig, graph: DependencyGraph, application?: string): { readonly facts: NormalizedGraphFacts; readonly layers: LayerReport } {
  ...
  return { layers, facts: {
    firstPartyModules: selected?.size ?? layers.summary.firstPartyFiles, applicationModules: layers.summary.applicationFiles,
    productionLines: layers.components.reduce((sum, component) => sum + component.lines, 0),
    edges: scopedEdges.length, sccs: layers.summary.applicationSccs, cyclicSccs: layers.summary.cyclicSccs,
    maximumLayer: layers.summary.maximumLayer, dynamicImports: scopedEdges.filter((edge) => edge.dynamic).length,
...
export interface ArchitectureSummary {
  ...
  readonly graph: {
    readonly productionModules: number; readonly productionLines: number; readonly edges: number;
    readonly sccs: number; readonly cyclicSccs: number; readonly maximumLayer: number;
    readonly dynamicImports: number; readonly unresolvedImports: number;
  };
  readonly candidates: Availability<{
    readonly total: number; readonly eligible: number; readonly blocked: number;
    readonly recommended: number; readonly reviewRequired: number; readonly discouraged: number;
  }>;
}
...
export function deriveAssessmentReports(snapshot: AssessmentSnapshot, input: {
  readonly limit?: number;
  ...
  const summary: ArchitectureSummary = {
    schemaVersion: 1, baseline: snapshot.baseline, qualification: snapshot.qualification,
    graph: { productionModules: normalized.facts.applicationModules, productionLines: normalized.facts.productionLines,
      edges: normalized.facts.edges, sccs: normalized.facts.sccs, cyclicSccs: normalized.facts.cyclicSccs,
      maximumLayer: normalized.facts.maximumLayer, dynamicImports: normalized.facts.dynamicImports, unresolvedImports: normalized.facts.unresolvedImports },
    candidates: candidateTotals,
  };
  const portfolioRecords = portfolio === undefined ? [] : [...portfolio.candidates]
    .sort((left, right) => right.score - left.score || byCodeUnit(left.id, right.id));
  const backlogRecords = portfolio === undefined ? [] : blockedCandidates(portfolio)
    .slice().sort((left, right) => right.lineCount - left.lineCount || byCodeUnit(left.id, right.id));
  const portfolioReport = portfolio === undefined ? unavailable<BoundedReport<PortfolioCandidate>>(candidateDiagnostics)
    : available(bound(portfolioRecords, limit, `assess --app ${snapshot.application} --evidence-dir <path> --full-portfolio`));
  const fullPortfolio = input.fullPortfolio !== true ? undefined
    : portfolio === undefined ? unavailable<readonly PortfolioCandidate[]>(candidateDiagnostics) : available(portfolioRecords);
  const hotspotReport = portfolio === undefined ? unavailable<BoundedReport<CouplingHotspot>>(candidateDiagnostics)
    : available(bound(analyzeCouplingHotspots(snapshot.graph, portfolio, snapshot.application), limit, `assess --app ${snapshot.application} --evidence-dir <path> --limit ${limit * 2}`));
  const backlogReport = portfolio === undefined ? unavailable<BoundedReport<PortfolioCandidate>>(candidateDiagnostics)
    : available(bound(backlogRecords, limit, `assess --app ${snapshot.application} --evidence-dir <path> --limit ${limit * 2}`));
  return { summary, layers, hotspots: hotspotReport, portfolio: portfolioReport, ...(fullPortfolio === undefined ? {} : { fullPortfolio }), backlog: backlogReport, findings: renderFindings(summary, layers, hotspotReport, portfolioReport, backlogReport, input.fullPortfolio === true, snapshot.application, input.batch) };
}

function portfolioTotals(portfolio: Portfolio) {
  const eligible = portfolio.candidates.filter((candidate) => candidate.eligible);
  return {
    total: portfolio.candidates.length, eligible: eligible.length, blocked: portfolio.candidates.length - eligible.length,
...
function bound<T>(records: readonly T[], limit: number, deeperCommand: string): BoundedReport<T> {
  ...
  return { total: records.length, limit, truncated: records.length > limit, omitted: Math.max(0, records.length - bounded.length), records: bounded, deeperCommand };
}
...
function renderFindings(
  ...
  const lines = [
    "# Architecture Assessment", "", "## Evidence boundary", "",
    ...
    "", "### Qualification limitations", "", ...qualificationLines(summary),
    "", "## Graph", "",
    ...
    "", "## Domains and cycles", "",
    ...layers.domains.slice().sort((left, right) => byCodeUnit(left.domain, right.domain)).map((domain) => `- ${domain.domain}: ${domain.files} files, ${domain.lines} lines, ${domain.crossDomainEdges.length} outgoing cross-domain edges`),
    ...domainFallback(layers), ...cycleLines(layers),
    "", "## Hotspots", "", ...availabilityLines(hotspots, (report) => report.records.map((entry) => `- ${entry.path}: pressure ${entry.pressureScore}, suggested ${entry.suggestedAction}`)),
    "", "## Candidate classifications", "", ...availabilityLines(portfolio, (report) => report.records.map((entry) => `- ${entry.id}: mechanically ${entry.eligible ? "eligible" : "blocked"}; classification ${entry.classification ?? "unclassified"}; recommendation ${entry.recommendation?.status ?? "unclassified"}${entry.recommendation?.cohesion === "low" ? "; low cohesion requires architectural review" : ""}`)),
    "", "## Preparation backlog", "", ...availabilityLines(backlog, (report) => report.records.map((entry) => `- ${entry.id}: ${entry.rejectionReasons.map((reason) => reason.code).sort(byCodeUnit).join(", ")}`)),
    "", "## Declaration splits", "", ...splitLines(batch),
    "", "## Evidence retained and omitted", "", ...evidenceLines(portfolio, hotspots, backlog, fullPortfolio, application),
    "", "## Reproduce", "", `Run \`assess --app ${application} --evidence-dir <path>\` against matching authoritative inputs.`,
...
function cycleLines(layers: LayerReport): string[] {
  const cycles = layers.components.filter((component) => component.cyclic)
    .slice().sort((left, right) => byCodeUnit(left.nodes.join("\0"), right.nodes.join("\0")));
...
function splitLines(batch: DeclarationBatchResult | undefined): string[] {
  ...
  return [
    ...
    ...batch.aggregate.entries.map((entry) => `- ${entry.sourcePath}: ${entry.declarationCount} declarations, ${entry.splitCandidateCount} candidates, ${entry.cycleCount} cycle(s), affinities ${entry.dominantAffinities.length === 0 ? "none" : entry.dominantAffinities.join(", ")}; detail ${entry.reportPath}`),
    ...(batch.aggregate.diagnostics.length === 0 ? [] : batch.aggregate.diagnostics.map((entry) => `- ${entry.category} ${entry.code}: ${entry.message}`)),
  ];
}

function evidenceLines(
  portfolio: AssessmentReports["portfolio"], hotspots: AssessmentReports["hotspots"], backlog: AssessmentReports["backlog"],
  fullPortfolio: boolean, application: string,
): string[] {
  return [
    "Raw scanner reports and the input inventory are mandatory replay authority and are retained.",
    ...(portfolio.status === "available" && portfolio.value.truncated ? [`- ${portfolio.value.omitted} portfolio record(s) omitted; use \`${portfolio.value.deeperCommand}\`.`] : []),
    ...(!fullPortfolio ? [`- Full portfolio details omitted by default; use \`assess --app ${application} --evidence-dir <path> --full-portfolio\`.`] : []),
    ...(hotspots.status === "available" && hotspots.value.truncated ? [`- ${hotspots.value.omitted} hotspot record(s) omitted; use \`${hotspots.value.deeperCommand}\`.`] : []),
    ...(backlog.status === "available" && backlog.value.truncated ? [`- ${backlog.value.omitted} backlog record(s) omitted; use \`${backlog.value.deeperCommand}\`.`] : []),
  ];
}
