...
export function preparationCommandSpecs(handlers: PreparationHandlers): Record<string, CommandSpec> {
  return {
    seams: { summary: "compile a read-only declaration-SCC seam proposal", usage: "seams --file <path> --candidate <id> [--target <path>] [--app <name>]", details: "Reports moved and retained groups, boundary imports, consumers, cycles, blockers, and type-only preparation eligibility without writing files.", run: handlers.seams },
    "seams-multi": { summary: "compile read-only declaration SCCs across explicit source files", usage: "seams-multi --file <path> --file <path> [--app <name>]", details: "Requires at least two configured application files and reports exact cross-file symbol edges, merged declarations, SCCs, and affected consumers.", run: handlers.multiFileSeams },
    "prepare-plan": { summary: "compile a type-only declaration preparation plan", usage: "prepare-plan --file <path> --candidate <id> --target <path> --module-specifier <specifier> --group <id> [--group <id> ...] [--out <path>] [--write]", details: "Only explicitly reviewed, safely type-only groups are accepted. Commit the written manifest with its rendered plan subject before prepare-apply --commit.", run: handlers.preparePlan },
    "prepare-multi-plan": { summary: "compile one atomic multi-file type preparation", usage: "prepare-multi-plan --spec <path> [--out <path>] [--write]", details: "The JSON spec names one reviewed multi-file candidate plus per-donor candidate, groups, target, and module specifier. Compilation requires exact atomic coverage and emits one replayable manifest; it never edits source.", run: handlers.prepareMultiPlan },
    "prepare-audit": { summary: "audit an applied declaration-preparation plan", usage: "prepare-audit --plan <path>", details: "Independently rescans baseline graph evidence and replays the exact declaration, import, and public-surface recipe.", run: handlers.prepareAudit },
    "prepare-apply": { summary: "simulate or apply a reviewed declaration-preparation plan", usage: "prepare-apply --plan <path> [--commit]", details: "Without --commit, runs replay, audit, and configured preparation gates in isolation. With --commit, requires the approved manifest commit and audits immediately.", run: handlers.prepareApply },
