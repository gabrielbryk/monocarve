...
async function apply(args: ParsedArgs): Promise<void> {
  ...
  if (flagBool(args, "refresh-if-baseline-only")) {
    throw new UsageError("--refresh-if-baseline-only is no longer supported; run refresh --plan <path> --out <new-path> --write, then approve the new manifest");
  }
...
export const transactionCommands: Record<string, CommandSpec> = {
  ...
  apply: { summary: "land a plan with one mandatory simulation", usage: "apply --plan <path> [--commit] [--resume] [--skip-gates] [--verify-lockfile]", details: "Use --commit as the default landing path: it simulates once, then immediately applies the identical verified journal. Without --commit, apply is a standalone feasibility or review run; do not run it immediately before a committed apply because --commit must simulate again and no simulation evidence is cached. Refresh is a separate command that writes a distinct manifest for review; apply never replaces approved bytes. --resume continues only from a verified transaction boundary; --skip-simulation is refused.", run: apply },
