...
async function reconcile(args: ParsedArgs): Promise<void> {
  ...
  if (!chain.valid || chain.phase !== "applied" || chain.approvalCommit === undefined || (chain.moveCommit === undefined && chain.wiringCommit === undefined) || chain.appliedCommit === undefined) {
  ...
  const record = compileReconciliationRecord({
    manifest, manifestBytes, manifestPath: path, approvalCommit: chain.approvalCommit,
    ...(chain.moveCommit === undefined ? {} : { moveCommit: chain.moveCommit }), ...(chain.wiringCommit === undefined ? {} : { wiringCommit: chain.wiringCommit }),
    observedHead: head, observedCommitDate: resolveCommit(rootDir, head).committedAt, audit, discrepancies,
    reason, approval: { subject: approvalSubject },
  });
...
async function receipt(args: ParsedArgs): Promise<void> {
  ...
  const approval = reconciliation === undefined ? undefined : verifyReconciliationLink({ rootDir, manifest, manifestPath: path, manifestBytes, chain, evidence: reconciliation });
  if (reconciliation === undefined && head !== chain.appliedCommit) throw new PreflightError("unreconciled receipt must be created at the exact application result commit");
  if (approval !== undefined && head !== approval.approvalCommit) throw new PreflightError("reconciled receipt must be created at the exact reconciliation approval commit");
  ...
  const receiptValue = compileAppliedPlanReceipt({
    ...
    observedCommit: head, observedCommitDate: resolveCommit(rootDir, head).committedAt, audit, manifest,
    ...(reconciliation === undefined || approval === undefined ? {} : { record: {
      path: reconciliation.path, bytes: reconciliation.bytes, value: reconciliation.value, approvalCommit: approval.approvalCommit,
    } }),
  });
...
function observedDiscrepancies(rootDir: string, manifest: Awaited<ReturnType<typeof loadManifest>>["manifest"]): { path: string; actual: ReturnType<typeof fileState> }[] {
  ...
  return [...expected].flatMap(([path, expectedState]) => {
...
function assertCurrentDiscrepancies(rootDir: string, manifest: Awaited<ReturnType<typeof loadManifest>>["manifest"], record: Awaited<ReturnType<typeof readReconciliationRecord>>["value"]): void {
  const actual = observedDiscrepancies(rootDir, manifest);
  if (actual.length !== record.discrepancies.length || actual.some((entry, index) => entry.path !== record.discrepancies[index]?.path || entry.actual !== record.discrepancies[index]?.actual)) {
...
export const reconciliationCommands: Record<string, CommandSpec> = {
  reconcile: {
    ...
    details: "Preview by default. Records only declared byte drift after proving the original approval and exact application chain. --write exclusively creates a new linked record and never changes the approved plan.",
    run: reconcile,
  },
  receipt: {
    summary: "compile an immutable applied-plan receipt",
    usage: "receipt --plan <path> [--reconciliation <approved-record>] [--out <path>] [--write]",
    details: "Preview by default. Requires the exact application-result commit and a passing independent audit. --write exclusively creates a new receipt and never changes the approved plan.",
    run: receipt,
  },
  "reconcile-approve": {
    summary: "inspect or commit one reconciliation record approval",
    usage: "reconcile-approve --record <path> [--commit]",
    details: "Without --commit, reports the reviewed record boundary. --commit creates an exact record-only approval directly atop the observed head and verifies hook output.",
