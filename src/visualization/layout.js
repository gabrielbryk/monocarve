...
async function layoutOwner(owner, nodes, edges) {
  const nestedGroups = group(nodes.filter((node) => nestedFor(node)), nestedFor);
  const nestedLayouts = [];
  ...
  const localEdges = aggregateEdges(edges.filter((edge) => memberIds.has(edge.from) && memberIds.has(edge.to) && itemId(edge.from) !== itemId(edge.to)), itemId);
  const local = await layoutFlat(items, localEdges, 210, 100);
  return { id: `owner:${owner}`, label: owner, members: nodes.map((node) => node.id), local, children: nestedLayouts, width: local.width + 80, height: local.height + 105 };
}
...
async function layoutFlat(items, edges, layerGap, nodeGap) {
  ...
  const result = await elk.layout({
    ...
    layoutOptions: {
      ...
      "elk.spacing.nodeNode": String(nodeGap), "elk.layered.spacing.nodeNodeBetweenLayers": String(layerGap),
      "elk.padding": "[top=25,left=25,bottom=25,right=25]", "elk.separateConnectedComponents": "true",
      "elk.spacing.componentComponent": String(nodeGap * 1.5),
    },
  });
  return { width: result.width ?? 1, height: result.height ?? 1, positions: new Map((result.children ?? []).map((child) => [child.id, { x: child.x ?? 0, y: child.y ?? 0 }])) };
}
...
function center(position, size, offsetX, offsetY) { return { x: offsetX + position.x + size.width / 2, y: offsetY + position.y + size.height / 2 }; }
function asBox(layout) { return { id: layout.id, width: layout.width, height: layout.height }; }
function asLeaf(node) { return { id: node.id, ...asLeafSize(node.id, node.label) }; }
function asLeafSize(id, label = id) { return { width: Math.max(150, Math.min(300, 44 + label.length * 9)), height: 52 }; }
...
function ownerFor(node) { return node.applications[0] ?? node.owners[0] ?? node.zones[0] ?? "unclassified"; }
