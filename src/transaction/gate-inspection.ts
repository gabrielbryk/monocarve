...
async function inspectOne(
  options: { config: MonocarveConfig; rootDir: string; manifest: ExtractionManifest },
  ...
  const worktree = await createWorktree({
    ...
    label: `${manifest.planId}-inspect-${index}`,
  });
  try {
    ...
    const run = await runGateTiers({
      gates: { package: tier === "package" ? [command] : [], project: tier === "project" ? [command] : [], workspace: tier === "workspace" ? [command] : [] },
      maxConcurrency: 1,
    ...
    return {
      ...
      changedPaths,
      undeclaredPaths,
      suggestedArtifacts: undeclaredPaths.map((path) => ({ path, regenerate: command, triggers: [...manifest.changedFiles] })),
      ...(result.exitCode === 0 ? {} : { diagnostic: result }),
    };
  } finally {
