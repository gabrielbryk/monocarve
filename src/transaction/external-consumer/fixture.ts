...
export function writeExternalConsumerFixture(root: string, target: ExtractionManifest["target"]): string {
  const lines = [
    ...
    ...(target.publicModules ?? []).flatMap((module, index) => importAssertions(module.specifier, module.requiredExports, `Subpath${index}_`)),
  ];
...
function importAssertions(specifier: string, exports: readonly { readonly name: string; readonly typeOnly: boolean }[], prefix: string): string[] {
  return exports.flatMap((entry, index) => importAssertion(specifier, entry, `${prefix}${index}`));
}

function importAssertion(specifier: string, entry: { readonly name: string; readonly typeOnly: boolean }, suffix: string): string[] {
  ...
  return entry.typeOnly ? [`import type ${statement} from ${packageName};`] : [`import ${statement} from ${packageName};`, `void ${alias};`];
}

function defaultAssertion(packageName: string, alias: string, typeOnly: boolean): string[] {
  return typeOnly ? [`import type ${alias} from ${packageName};`] : [`import ${alias} from ${packageName};`, `void ${alias};`];
}
