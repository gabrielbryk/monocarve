...
export function currentGeneratorManifest(options: {
  ...
  for (const [id, records] of groups) {
    ...
    const evolution = findEvolution({ ...options, command: configured.command, ...(configured.verify === undefined ? {} : { verify: configured.verify }), outputs: newOutputs });
...
function findEvolution(options: {
  ...
  for (const [index, commit] of commits.entries()) {
    ...
    for (const repositoryPath of changed.filter((path) => path.endsWith(".json"))) {
      ...
      try { candidate = JSON.parse(text); } catch { continue; }
      try { assertPreparerManifest(options.config, candidate); } catch { continue; }
      const manifest = candidate as PreparerManifest;
      if (manifest.baseline.commit !== parent || manifest.bootstrapConfig?.path !== configPath || manifest.preparer.command !== options.command || manifest.preparer.verify !== options.verify) continue;
      ...
      if (outputCommit === undefined || git({ cwd: options.rootDir }, "rev-parse", `${outputCommit}^`) !== commit || git({ cwd: options.rootDir }, "log", "-1", "--format=%s", outputCommit) !== manifest.preparer.commit.subject) continue;
