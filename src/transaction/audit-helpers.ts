...
export function unauditableManifest(config: MonocarveConfig, manifest: ExtractionManifest): string[] {
  ...
  if (
    value["schemaVersion"] !== LEGACY_PLAN_SCHEMA_VERSION &&
    value["schemaVersion"] !== PREVIOUS_PLAN_SCHEMA_VERSION &&
  ...
  const require = (key: string, ok: boolean, kind: string): void => {
    if (!ok) failures.push(`[manifest-shape] ${key} must be ${kind}`);
  };
  ...
  require("sourceBlobs", isRecord("sourceBlobs"), "an object");
  require("operations", isArray("operations"), "an array");
  require("consumers", isArray("consumers"), "an array");
  require("generatedFiles", isArray("generatedFiles"), "an array");
  require("evaluationEffects", isArray("evaluationEffects"), "an array");
  require("source", isRecord("source"), "an object");
  ...
  if (isRecord("target")) {
    ...
    require("target.packageRoot", typeof target["packageRoot"] === "string", "a string");
    require("target.entrypoint", typeof target["entrypoint"] === "string", "a string");
    require("target.packageName", typeof target["packageName"] === "string", "a string");
    require("target.requiredExports", Array.isArray(target["requiredExports"]), "an array");
  }
  require("expectedDynamicImportDelta", isRecord("expectedDynamicImportDelta"), "an object");
  if (isRecord("expectedDynamicImportDelta")) {
    const delta = value["expectedDynamicImportDelta"] as Record<string, unknown>;
    require("expectedDynamicImportDelta.added", Array.isArray(delta["added"]), "an array");
    require("expectedDynamicImportDelta.removed", Array.isArray(delta["removed"]), "an array");
  }
  ...
  if ("boundaryBaseline" in value) {
    ...
    if (!isRecord("boundaryBaseline") || !Array.isArray((record as { edges?: unknown }).edges) || typeof (record as { digest?: unknown }).digest !== "string") {
      ...
      (record as { edges: unknown[] }).edges.some(
        (edge) =>
          typeof edge !== "object" ||
          edge === null ||
  ...
  if (evacuation !== undefined) {
    const sourceFiles =
      isRecord("source") && Array.isArray((value["source"] as Record<string, unknown>)["files"]) ? (value["source"] as { files: string[] }).files : [];
    ...
    const canonical = (paths: readonly string[]): boolean =>
      new Set(paths).size === paths.length && paths.every((path, index) => index === 0 || path > paths[index - 1]!);
    ...
    if (evacuation.id !== expected || (manifest.planId !== expected && !manifest.planId.startsWith(`${expected}--`)))
