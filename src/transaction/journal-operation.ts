...
function applyLockfileImporter(adapter: PackageManagerAdapter, operation: Extract<PlanOperation, { kind: "lockfile-importer" }>, root: string): void {
  ...
  if (operation.mode === "delete") {
    if (adapter.lockfileImporterHash(next, operation.packageRoot) !== undefined)
      throw new JournalError(`lockfile importer deletion did not remove ${operation.packageRoot}`);
...
function applyPathMigration(config: MonocarveConfig, operation: Extract<PlanOperation, { kind: "migrate-path-keys" }>, root: string): void {
  const path = resolve(root, operation.path);
