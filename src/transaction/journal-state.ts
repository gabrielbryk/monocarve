...
function blockState(
  ...
  return {
    current: text === "" ? undefined : adapter.lockfileImporterHash(text, operation.packageRoot),
    expected: hashText(operation.block),
  };
}
...
export function isAtPrecondition(adapter: PackageManagerAdapter, operation: PlanOperation, root: string): boolean {
  ...
  return stateAt(root, operation.kind === "write-file" || operation.kind === "migrate-path-keys" ? operation.path : operation.file) === operation.preconditionHash;
}
