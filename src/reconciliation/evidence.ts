...
export interface ReadEvidence<T> { readonly path: string; readonly bytes: string; readonly value: T }
...
function canonical(value: unknown): string { return `${stableStringify(value, 2)}\n`; }
...
function read<T>(rootDir: string, inputPath: string, parse: (value: unknown) => T, serialize: (value: T) => string): ReadEvidence<T> {
  ...
  let raw: unknown;
  try { raw = JSON.parse(bytes); } catch { throw new ReconciliationValidationError(`evidence is not JSON: ${path}`); }
...
export function verifyReconciliationLink(options: ReconciliationLinkOptions): { readonly approvalCommit: string } {
  ...
  if (record.plan.planId !== options.manifest.planId || record.plan.path !== options.manifestPath || record.plan.digest !== hashText(options.manifestBytes) ||
      record.plan.baselineCommit !== options.manifest.baselineCommit || record.plan.approvalCommit !== chain.approvalCommit) fail("reconciliation plan linkage does not match the approved manifest");
  if (record.generator.name !== options.manifest.generator.name || record.generator.version !== options.manifest.generator.version ||
      stableStringify(record.provenance) !== stableStringify(options.manifest.provenance)) fail("reconciliation provenance does not match the approved manifest");
  if (record.application.moveCommit !== chain.moveCommit || record.application.wiringCommit !== chain.wiringCommit || record.application.resultingCommit !== chain.appliedCommit) {
  ...
  const children = (tryGit({ cwd: options.rootDir }, "rev-list", "--first-parent", "--reverse", `${record.observed.headCommit}..HEAD`) ?? "").split("\n").filter(Boolean);
  const commit = children[0];
  if (commit === undefined || tryGit({ cwd: options.rootDir }, "rev-parse", `${commit}^`) !== record.observed.headCommit) fail("reconciliation approval is not directly atop its observed head");
  if (tryGit({ cwd: options.rootDir }, "log", "-1", "--format=%s", commit) !== record.approval.subject) fail("reconciliation approval subject does not match the record");
  ...
  if (showBaseline(options.rootDir, commit, options.evidence.path) !== options.evidence.bytes) fail("committed reconciliation bytes do not match the reviewed record");
  return { approvalCommit: commit };
}

export function verifyReceiptLink(receipt: ReadEvidence<AppliedPlanReceipt>, manifest: ExtractionManifest, manifestPath: string, manifestBytes: string, chain: CommitChainEvidence): void {
  ...
  if (value.plan.planId !== manifest.planId || value.plan.path !== manifestPath || value.plan.digest !== hashText(manifestBytes) ||
      value.plan.baselineCommit !== manifest.baselineCommit || value.plan.approvalCommit !== chain.approvalCommit) fail("receipt plan linkage does not match the approved manifest");
  if (value.application.moveCommit !== chain.moveCommit || value.application.wiringCommit !== chain.wiringCommit || value.application.resultingCommit !== chain.appliedCommit) {
    fail("receipt application linkage does not match the exact chain");
  }
  if (value.generator.name !== manifest.generator.name || value.generator.version !== manifest.generator.version || stableStringify(value.provenance) !== stableStringify(manifest.provenance)) {
