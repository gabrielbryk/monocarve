...
export function classifyTypeOnlyExtraction(input: ClassifyTypeOnlyExtractionInput): TypeOnlyExtractionSafety {
  ...
  if (!group) return refused(input.groupId, [], [{
    code: "unknown-declaration-group",
    message: `declaration group ${input.groupId} is not present in the symbol graph`,
  }]);
  ...
  if (hashText(input.sourceText) !== input.graph.sourceHash) {
    evidence.push({
      code: "graph-source-mismatch",
      message: "source text does not match the symbol graph source hash",
    });
  }
  evidence.push(...selectionEvidence(group, input.selectedDeclarationIds));
  if (members.length !== group.declarationIds.length) {
    evidence.push({
      code: "incomplete-declaration-group",
      message: `group ${group.name} references declaration identities absent from the graph`,
    });
  }
  if (group.space === "both") {
    evidence.push({
      code: "type-value-mixed-group",
      message: `group ${group.name} occupies both TypeScript declaration spaces`,
    });
  }
  ...
  if (!sameSet(expectedSpans, actualSpans)) {
    evidence.push({
      code: "incomplete-declaration-group",
      message: `group ${group.name} does not cover every same-name top-level declaration in source`,
    });
  }
...
function selectionEvidence(group: DeclarationGroup, selected: readonly Sha256[] | undefined): TypeOnlyExtractionEvidence[] {
  ...
  return sameSet(new Set(expected), new Set(actual)) ? [] : [{
    code: "incomplete-declaration-group",
    message: `selection must include every declaration in merged group ${group.name}`,
  }];
}
...
function nodeRefusals(node: ts.Declaration, declaration: SymbolDeclaration, sourceFile: ts.SourceFile): TypeOnlyExtractionEvidence[] {
  ...
  if (has(ts.SyntaxKind.DefaultKeyword)) return [{
    code: "default-export", message: `default export ${declaration.name} cannot retain a compatible named re-export`, ...at,
  }];
  if (ts.isModuleDeclaration(node)) {
    if (ts.isStringLiteral(node.name)) return [{
      code: "module-augmentation", message: "string-literal module declarations cannot be extracted as type-only declarations", ...at,
    }];
    return [{
  ...
  if (ts.isEnumDeclaration(node)) return [{
    code: "enum-declaration", message: `enum ${declaration.name} is both a type and a runtime value`, ...at,
  }];
  if (has(ts.SyntaxKind.DeclareKeyword) && !ts.isInterfaceDeclaration(node) && !ts.isTypeAliasDeclaration(node)) return [{
    code: "ambient-declaration", message: `ambient declaration ${declaration.name} is outside the writable preparer's scope`, ...at,
  }];
  if (has(ts.SyntaxKind.DeclareKeyword) && !declaration.exported) return [{
  ...
  if (ts.isFunctionDeclaration(node)) return [{
    code: "function-overload-group", message: `function declaration ${declaration.name} may be an overload group and is runtime-emitting`, ...at,
  }];
  if (declaration.space !== "type") return [{
    code: "non-type-declaration", message: `${declaration.kind} ${declaration.name} is not type-only`, ...at,
  }];
  return [];
}
...
function isNamedDeclaration(node: ts.Statement): node is ts.ClassDeclaration | ts.EnumDeclaration | ts.FunctionDeclaration | ts.InterfaceDeclaration | ts.ModuleDeclaration | ts.TypeAliasDeclaration {
  return ts.isClassDeclaration(node) || ts.isEnumDeclaration(node) || ts.isFunctionDeclaration(node)
    || ts.isInterfaceDeclaration(node) || ts.isModuleDeclaration(node) || ts.isTypeAliasDeclaration(node);
}
...
function compareEvidence(left: TypeOnlyExtractionEvidence, right: TypeOnlyExtractionEvidence): number {
  return (left.start ?? -1) - (right.start ?? -1) || (left.end ?? -1) - (right.end ?? -1)
    || byCodeUnit(left.code, right.code) || byCodeUnit(left.message, right.message);
}
