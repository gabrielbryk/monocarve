...
interface SelectedGroup { readonly group: DeclarationGroup; readonly analysis: WorkspaceSymbolAnalysis }
...
export function planMultiFileSeams(input: PlanMultiFileSeamsInput): MultiFileSeamPlan {
  ...
  const analyses = paths.map((sourcePath) => analyzeWorkspaceSymbols({
    rootDir: input.rootDir, tsconfigPath: input.tsconfigPath, sourcePath, affinityForPath: input.affinityForPath,
  }));
  ...
  const candidates = components(analyses, edges, mergedSymbols).map(
    (ids) => candidateFor(ids, analyses, edges, input.affinityForPath, input.targetPaths),
  );
  return planResult(sourceHashes, edges, mergedSymbols, candidates, []);
}

function planResult(sourceHashes: Readonly<Record<string, Sha256>>, edges: readonly MultiFileSymbolEdge[], mergedSymbols: readonly MultiFileMergedSymbol[], candidates: readonly MultiFileSeamCandidate[], blockers: readonly SeamBlocker[]): MultiFileSeamPlan {
...
function staleBlockers(actual: Readonly<Record<string, Sha256>>, expected?: Readonly<Record<string, Sha256>>): SeamBlocker[] {
  ...
  for (const path of Object.keys(actual).sort(byCodeUnit)) {
    if (expected[path] !== actual[path]) blockers.push({ code: "stale-source", message: `${path} changed since its source hash was observed`, confidence: "exact" });
  }
  return blockers;
}
...
function addReference(node: ts.Identifier, file: ts.SourceFile, sourcePath: string, analysis: WorkspaceSymbolAnalysis, absoluteToPath: ReadonlyMap<string, string>, selected: ReadonlyMap<string, WorkspaceSymbolAnalysis>, checker: ts.TypeChecker, output: Map<string, { edge: Omit<MultiFileSymbolEdge, "referenceCount">; count: number }>): void {
  ...
  const edge = { sourcePath, sourceGroupId: sourceGroup.id, sourceName: sourceGroup.name, targetPath, targetGroupId: targetGroup.id, targetName: targetGroup.name, space, confidence: "exact" as const };
...
function tarjan(ids: readonly Sha256[], adjacency: ReadonlyMap<Sha256, ReadonlySet<Sha256>>): Sha256[][] {
  let next = 0; const stack: Sha256[] = []; const onStack = new Set<Sha256>(); const index = new Map<Sha256, number>(); const low = new Map<Sha256, number>(); const result: Sha256[][] = [];
  const visit = (id: Sha256): void => {
    index.set(id, next); low.set(id, next); next += 1; stack.push(id); onStack.add(id);
    ...
    const component: Sha256[] = []; let current: Sha256;
...
function candidateFor(ids: readonly Sha256[], analyses: readonly WorkspaceSymbolAnalysis[], edges: readonly MultiFileSymbolEdge[], affinityForPath: (path: string) => string, targets?: Readonly<Record<string, string>>): MultiFileSeamCandidate {
  const selected = new Set(ids); const records = groupRecords(analyses).filter((entry) => selected.has(entry.group.id));
  ...
  const outsideFiles = records.flatMap(({ group, analysis }) => analysis.consumers.filter(
    (item) => item.groupId === group.id && !sourcePaths.includes(item.consumerPath),
  ));
  const insideFilesOutsideComponent = edges.filter(
    (edge) => selected.has(edge.targetGroupId) && !selected.has(edge.sourceGroupId),
  ).map((edge): ExternalSymbolConsumer => ({
    groupId: edge.targetGroupId, groupName: edge.targetName, consumerPath: edge.sourcePath,
    affinity: affinityForPath(edge.sourcePath), space: edge.space, referenceCount: edge.referenceCount,
  }));
  const affectedConsumers = deduplicateConsumers([...outsideFiles, ...insideFilesOutsideComponent]).sort(compareConsumers);
  const selectedTargets = [...new Set(ids.map((id) => targets?.[id]).filter((path): path is string => path !== undefined).map((path) => normalizedTarget(path)))].sort(byCodeUnit);
  const blockers: SeamBlocker[] = [];
  if (selectedTargets.length > 1) blockers.push({ code: "target-collision", message: "groups in one atomic component have conflicting target paths", confidence: "exact" });
  ...
  return {
    id: hashJson({ ids, sourcePaths }), groupIds: [...ids], groups, sourcePaths, cyclic,
    placement: selectedTargets[0] === undefined ? { confidence: "heuristic" } : { confidence: selectedTargets.length === 1 ? "exact" : "heuristic", targetPath: selectedTargets[0] },
    affectedConsumers, blockers,
  };
}

function groupRecords(analyses: readonly WorkspaceSymbolAnalysis[]): SelectedGroup[] { return analyses.flatMap((analysis) => analysis.source.groups.map((group) => ({ group, analysis }))); }
...
function mergedSymbolGroups(root: string, program: ts.Program, analyses: readonly WorkspaceSymbolAnalysis[]): MultiFileMergedSymbol[] {
  ...
  for (const analysis of analyses) for (const declaration of analysis.source.declarations) {
  ...
  const seen = new Set<ts.Symbol>(); const result: MultiFileMergedSymbol[] = [];
  for (const file of program.getSourceFiles()) for (const statement of file.statements) {
    ...
    seen.add(symbol);
    const groups = symbol.declarations?.map((declaration) => {
      let path: string;
      try { path = relativeWorkspacePath(root, declaration.getSourceFile().fileName); } catch { return undefined; }
    ...
    if (ids.length > 1) result.push({ groupIds: ids, sourcePaths: [...new Set(groups.map((group) => group.sourcePath))].sort(byCodeUnit), name: name.text, confidence: "exact" });
  }
  return result.sort((a, b) => byCodeUnit(a.name, b.name) || byCodeUnit(a.groupIds[0] ?? "", b.groupIds[0] ?? ""));
}
function normalizedTarget(path: string): string { const result = normalizePath(path); if (!result || result === "." || isAbsolute(result) || result.split("/").includes("..")) throw new SeamPlanningError(`invalid target path ${path}`); return result; }
function createProgram(root: string, config: string): ts.Program { const path = workspacePath(root, relativeWorkspacePath(root, config)); const read = ts.readConfigFile(path, ts.sys.readFile); if (read.error) throw new SeamPlanningError(`cannot read TypeScript config ${config}`); const parsed = ts.parseJsonConfigFileContent(read.config, ts.sys, dirname(path), undefined, path); if (parsed.errors.length) throw new SeamPlanningError(`cannot build TypeScript program from ${config}`); return ts.createProgram({ rootNames: parsed.fileNames, options: parsed.options }); }
function containingDeclarationName(node: ts.Node, file: ts.SourceFile): string | undefined { let current: ts.Node | undefined = node; while (current?.parent && current.parent !== file) current = current.parent; return current && declarationName(current as ts.Declaration); }
function declarationName(node: ts.Declaration): string | undefined { const name = (node as ts.Declaration & { name?: ts.DeclarationName }).name; return name && ts.isIdentifier(name) ? name.text : undefined; }
function isDeclarationName(node: ts.Identifier): boolean { return (node.parent as ts.Declaration & { name?: ts.Node }).name === node; }
function normalize(path: string): string { return path.replaceAll("\\", "/"); }
function compareEdges(a: MultiFileSymbolEdge, b: MultiFileSymbolEdge): number { return byCodeUnit(a.sourcePath, b.sourcePath) || byCodeUnit(a.sourceGroupId, b.sourceGroupId) || byCodeUnit(a.targetPath, b.targetPath) || byCodeUnit(a.targetGroupId, b.targetGroupId) || byCodeUnit(a.space, b.space); }
function compareConsumers(a: ExternalSymbolConsumer, b: ExternalSymbolConsumer): number { return byCodeUnit(a.consumerPath, b.consumerPath) || byCodeUnit(a.groupId, b.groupId) || byCodeUnit(a.affinity, b.affinity); }
