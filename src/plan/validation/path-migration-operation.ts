...
export function validatePathMigrationOperation(
  ...
  const configured = triggeredPathMigrations(
    options.config,
    moves.map((move) => move.source),
  ).find((artifact) => artifact.path === operation.path);
  if (!configured) {
  ...
  if (operation.preconditionHash === operation.resultHash)
    issues.add("path-migration-identity", `path migration must change the artifact: ${operation.path}`, at);
