...
function buildManifest(state: BuildState, selection: ReturnType<typeof selectExtractionSources>, rewrites: ReadonlyMap<string, readonly EscapeRewrite[]>, operations: PlanOperation[]): ExtractionManifest {
  ...
  const provenance = {
    ...buildPlanProvenance({ config: state.config, profileGates: state.profile.gates, scaffoldTemplates: state.templates, packageManager: state.packageManager, taskRunner: state.taskRunner, ...(state.context.exists("package.json") ? { rootPackageJson: state.context.text("package.json") } : {}) }),
...
function productionSccs(candidateSccs: readonly { readonly id: string; readonly members: readonly string[] }[], production: readonly string[]): Record<string, readonly string[]> {
