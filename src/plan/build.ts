...
function buildManifest(state: BuildState, selection: ReturnType<typeof selectExtractionSources>, rewrites: ReadonlyMap<string, readonly EscapeRewrite[]>, operations: PlanOperation[]): ExtractionManifest {
  ...
  const provenance = buildPlanProvenance({ config: state.config, profileGates: state.profile.gates, scaffoldTemplates: state.templates, packageManager: state.packageManager, taskRunner: state.taskRunner, ...(state.context.exists("package.json") ? { rootPackageJson: state.context.text("package.json") } : {}) });
  return { schemaVersion: PLAN_SCHEMA_VERSION, planId: profilePlanId(state.candidate.id, state.profile.name), createdAt: state.baseline.committedAt, generator: { ...GENERATOR }, provenance, baselineCommit: state.baseline.commit, graphDigest: graphDigest(state.graph), application: state.candidate.application,
