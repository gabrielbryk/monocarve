...
export function verifyGeneratedSourceOperations(
  rootDir: string,
  baseline: string,
  ...
  for (const operation of operations) {
    if (operation.kind === "adopt-generated-source") {
      ...
      if (showBaselineBytes(rootDir, baseline, operation.declaredSource) !== null)
        failures.push(`declared generated source still exists: ${operation.declaredSource}`);
      ...
      if (bytes !== null) {
        ...
        if (hashText(removed.header) !== operation.removedHeader.hash || removed.rest !== operation.contents)
          failures.push(`generated-source adoption recipe does not reproduce ${operation.file.path}`);
      }
