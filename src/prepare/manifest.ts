...
function validateDeclaration(declaration: PreparationDeclarationSelector, group: PreparationDeclarationGroupSelector, add: AddIssue): void {
  ...
  if (
    ...
    add("declaration-hash", "declaration, selector, source, span, and extraction hashes must be SHA-256 values", declaration.sourcePath);
  if (declaration.sourcePath !== group.sourcePath || declaration.name !== group.name || declaration.space !== "type")
    add("declaration-group", "every group member must match its group source, name, and type-only space", declaration.sourcePath);
  ...
  if (declaration.kind !== "interface" && declaration.kind !== "type-alias")
    add("declaration-kind", "only interfaces and type aliases are eligible for type-only extraction", declaration.sourcePath);
  if (
    ...
    add("declaration-span", "declaration span must be a non-empty pair of UTF-16 offsets", declaration.sourcePath);
  if (
  ...
  if (declaration.selectorId !== selectorId(declaration))
    add("selector-id", `selector id for ${declaration.name} does not match its removal recipe`, declaration.sourcePath);
}
...
function validateTargetDeclarationProofs(operation: ExtractTypeDeclarationsOperation, add: AddIssue): void {
  ...
  for (const proof of operation.targetDeclarationProofs) {
    ...
    if (
      ...
      proof.targetEnd > operation.targetContents.length ||
      proof.targetExtractionStart < 0 ||
      proof.targetExtractionEnd < proof.targetExtractionStart ||
      proof.targetExtractionEnd > operation.targetContents.length ||
  ...
  const ordered = [...regions].toSorted(
    (left, right) =>
      ...
      byCodeUnit(left.selectorId, right.selectorId),
  );
  for (let index = 1; index < ordered.length; index += 1) {
    const previous = ordered[index - 1]!;
    const current = ordered[index]!;
    if (current.targetExtractionStart < previous.targetExtractionEnd)
...
function validateLiveState(manifest: PreparationManifest, options: ValidatePreparationManifestOptions, add: AddIssue): void {
  ...
  if (options.currentContents) {
    for (const group of manifest.declarations)
      for (const declaration of group.declarations) {
        ...
        if (hashText(contents) !== declaration.sourceHash)
          add("stale-selector", "current source hash differs from declaration selector", declaration.sourcePath);
...
function validateSortedStrings(values: readonly string[] | undefined, rule: string, label: string, add: AddIssue): void {
  ...
  for (let index = 1; index < values.length; index += 1)
    if (byCodeUnit(values[index - 1]!, values[index]!) >= 0) add(rule, `${label} must be sorted and unique`);
}
