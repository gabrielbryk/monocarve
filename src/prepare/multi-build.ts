...
export function compileMultiFilePreparationManifest(input: CompileMultiFilePreparationInput): PreparationManifest {
  ...
  const manifest = createPreparationManifest({
    ...
    gates: {
      package: union(manifests.map((item) => item.gates.package)),
      project: union(manifests.map((item) => item.gates.project)),
      workspace: union(manifests.map((item) => item.gates.workspace)),
    },
  });
