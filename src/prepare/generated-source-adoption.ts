...
export function compileGeneratedSourceAdoption(input: CompileGeneratedSourceAdoptionInput): PreparationManifest {
  ...
  const operations: PreparationReplayOperation[] = adoption.artifacts.map((artifact) => {
    ...
    if (showBaseline(input.rootDir, baseline.commit, artifact.missingSource) !== null)
  ...
  if (adoption.retireGenerator !== undefined) {
    ...
    const generatorNames = new Set([
      adoption.retireGenerator,
      ...adoption.artifacts.map((artifact) => {
        const owner = ownerFor(input.config, artifact.path);
        return owner === "" || !adoption.retireGenerator!.startsWith(`${owner}/`)
          ? adoption.retireGenerator!
    ...
    operations.push({
      ...
      file: {
        path: adoption.retireGenerator,
        preconditionHash: hashText(generatorText),
        preconditionMode: baselineFileMode(input.rootDir, baseline.commit, adoption.retireGenerator),
        resultHash: hashText(""),
        resultMode: 0,
      },
    });
