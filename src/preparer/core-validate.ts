...
function isInvalidReplacementRecord(item: ReplacementRecord): boolean {
  return item === null || typeof item !== "object" || typeof item.path !== "string" || typeof item.before !== "string" || typeof item.after !== "string" || (item.prefix !== undefined && typeof item.prefix !== "string") || (item.suffix !== undefined && typeof item.suffix !== "string");
}
...
function isInvalidCreateRecord(item: CreateRecord): boolean {
  return item === null || typeof item !== "object" || typeof item.path !== "string" || typeof item.contents !== "string" || (item.mode !== 0o644 && item.mode !== 0o755);
}
...
function isInvalidBootstrapConfig(bootstrapConfig: ManifestShape["bootstrapConfig"]): boolean {
  return bootstrapConfig === null || typeof bootstrapConfig !== "object" || typeof bootstrapConfig.path !== "string" || typeof bootstrapConfig.contents !== "string" || bootstrapConfig.resultHash !== hashText(bootstrapConfig.contents) || ![0o644, 0o755].includes(bootstrapConfig.preconditionMode as number) || ![0o644, 0o755].includes(bootstrapConfig.resultMode);
}
...
function assertManifestEnvelopeFields(manifest: ManifestShape): void {
  ...
  if (manifest.bootstrapConfig !== undefined && isInvalidBootstrapConfig(manifest.bootstrapConfig)) throw new PreparerError("preparer manifest bootstrap config is invalid");
...
function assertManifestPreparerRecordsShape(preparer: NonNullable<ManifestShape["preparer"]>): void {
  if (preparer.replacements !== undefined && (!Array.isArray(preparer.replacements) || preparer.replacements.some(isInvalidReplacementRecord))) throw new PreparerError("preparer manifest replacements are invalid");
  if (preparer.creates !== undefined && (!Array.isArray(preparer.creates) || preparer.creates.some(isInvalidCreateRecord))) throw new PreparerError("preparer manifest creates are invalid");
...
function assertManifestScopeShape(manifest: ManifestShape): void {
  ...
  if (!Array.isArray(manifest.mutations) || manifest.mutations.some(isInvalidMutationRecord)) throw new PreparerError("preparer manifest mutations are invalid");
