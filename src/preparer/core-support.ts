...
export function unique(items: readonly string[]): string[] { return [...new Set(items)].sort(byCodeUnit); }
export function validatedPath(root: string, path: string): string { workspacePath(root, path); return path.replaceAll("\\", "/"); }
...
export function expandDeclaredOutputs(policy: Pick<PreparerConfig, "outputs">, vars: Readonly<Record<string, string>>, mapPath: (path: string) => string): string[] {
...
export function expandCreatesPolicy(root: string, creates: PreparerConfig["creates"], vars: Readonly<Record<string, string>>): FileCreate[] | undefined {
  return creates?.map((create) => ({
    path: validatedPath(root, renderTemplate(create.path, vars)),
...
export function expandGeneratedArtifacts(config: MonocarveConfig, triggerPaths: readonly string[]) {
  return triggeredArtifacts(config, triggerPaths).map((artifact) => ({
...
export function canonicalMode(mode: number): 0o644 | 0o755 { return (mode & 0o111) === 0 ? 0o644 : 0o755; }
