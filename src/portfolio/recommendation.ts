...
export function recommendCandidate(
  ...
  if (
    (config.portfolio.maxRecommendedFiles !== undefined && candidate.files.length > config.portfolio.maxRecommendedFiles) ||
  ...
  const status = reasons.some((item) => discouraged.has(item.code)) ? "discouraged" : reasons.length > 0 ? "review-required" : "recommended";
