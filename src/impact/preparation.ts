...
import { executePreparationJournal, finalizeCompletedPreparationJournal, verifyPreparationOperations } from "../prepare/journal.ts";
...
export async function analyzePreparationImpact(options: {
  ...
  try {
    ...
    const before = buildPortfolio({
      ...
      graph: baselineGraph,
      context: new WorkspaceContext(options.config, worktree.workspacePath),
    ...
    return comparePortfolios(
      options.manifest.planId,
      baselineGraph.paths.map((path) => baselineGraph.nodes.get(path)!),
      projectedGraph.paths.map((path) => projectedGraph.nodes.get(path)!),
      before,
      after,
      options.application,
    );
...
export function comparePortfolios(
  ...
  const beforeRecommended = new Set(
    beforePortfolio.candidates.filter((candidate) => candidate.recommendation?.status === "recommended").map((candidate) => candidate.id),
  );
  const afterRecommended = new Set(
    afterPortfolio.candidates.filter((candidate) => candidate.recommendation?.status === "recommended").map((candidate) => candidate.id),
  );
  return {
...
function snapshot(
  nodes: readonly { zone: string; application?: string; lineCount: number }[],
  portfolio: Portfolio,
  application?: string,
  ...
  return {
    applicationLines: nodes
      .filter((node) => node.zone === "application" && (application === undefined || node.application === application))
      .reduce((sum, node) => sum + node.lineCount, 0),
